...
import { AI_TOOLS, APEWORKFLOW_DIR_NAME } from '../core/config.js';
...
function getCommandPath(command: Command): string {
  ...
  while (current) {
    ...
    // Skip the root 'apeworkflow' command
    if (name && name !== 'apeworkflow') {
      names.unshift(name);
    }
    current = current.parent;
  }

  return names.join(':') || 'apeworkflow';
}
...
async function hasRepoLocalApeWorkflowProject(projectPath: string): Promise<boolean> {
  try {
    const stats = await fs.stat(path.join(projectPath, APEWORKFLOW_DIR_NAME));
...
program
  ...
  .action(async (options?: { tool?: string; noInteractive?: boolean }) => {
    try {
      console.log('Note: "apeworkflow experimental" is deprecated. Use "apeworkflow init" instead.');
      const { InitCommand } = await import('../core/init.js');
      const initCommand = new InitCommand({
        tools: options?.tool,
...
program
  .command('update [path]')
  .description('Update ApeWorkflow instruction files')
  .option('--force', 'Force update even when tools are up to date')
  .action(async (targetPath = '.', options?: { force?: boolean }) => {
    try {
      const resolvedPath = path.resolve(targetPath);
      const updateCommand = new UpdateCommand({ force: options?.force });
      if (await hasRepoLocalApeWorkflowProject(resolvedPath)) {
      ...
      if (workspaceRoot) {
        throw new Error(
          'ApeWorkflow workspace detected. Run `apeworkflow workspace update` to refresh workspace-local guidance and skills.'
        );
      }
...
const changeCmd = program
  .command('change')
  .description('Manage ApeWorkflow change proposals');

// Deprecation notice for noun-based commands
changeCmd.hook('preAction', () => {
  console.error('Warning: The "apeworkflow change ..." commands are deprecated. Prefer verb-first commands (e.g., "apeworkflow list", "apeworkflow validate --changes").');
});
...
changeCmd
  .command('list')
  .description('List all active changes (DEPRECATED: use "apeworkflow list" instead)')
  .option('--json', 'Output as JSON')
  .option('--long', 'Show id and title with counts')
  .action(async (options?: { json?: boolean; long?: boolean }) => {
    try {
      console.error('Warning: "apeworkflow change list" is deprecated. Use "apeworkflow list".');
      const changeCommand = new ChangeCommand();
...
program
  ...
  .option('--concurrency <n>', 'Max concurrent validations (defaults to env APEWORKFLOW_CONCURRENCY or 6)')
  .option('--no-interactive', 'Disable interactive prompts')
...
const completionCmd = program
  .command('completion')
  .description('Manage shell completions for ApeWorkflow CLI');
...
// Set command group
const setCmd = program.command('set').description('Set checked-in ApeWorkflow metadata');
