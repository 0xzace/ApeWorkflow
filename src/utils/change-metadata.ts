...
const METADATA_FILENAME = '.apeworkflow.yaml';
...
/**
 * Writes change metadata to .apeworkflow.yaml in the change directory.
 *
...
/**
 * Reads change metadata from .apeworkflow.yaml in the change directory.
 *
 * @param changeDir - The path to the change directory
 * @param projectRoot - Optional project root for project-local schema resolution
 ...
 * 2. Schema from .apeworkflow.yaml metadata (if exists)
 * 3. Schema from apeworkflow/config.yaml (if exists)
...
export function resolveSchemaForChange(
  ...
  // Derive project root from changeDir (changeDir is typically projectRoot/apeworkflow/changes/change-name)
