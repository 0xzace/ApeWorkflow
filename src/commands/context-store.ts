...
async function confirmSetup(
  ...
  console.log('');
  console.log('ApeWorkflow will create:');
  console.log('');
...
async function confirmRemove(id: string, root: string, options: ContextStoreRemoveOptions): Promise<void> {
  ...
  if (options.json || !isInteractive()) {
    throw new ContextStoreError(
      ...
      {
        target: 'context_store.root',
        fix: `apeworkflow context-store remove ${id} --yes`,
      }
    );
  }
...
function printListHuman(payload: ContextStoreListOutput): void {
  if (payload.context_stores.length === 0) {
    ...
    console.log('');
    console.log('Next:');
    console.log('  apeworkflow context-store setup team-context');
    console.log('  apeworkflow context-store register /path/to/context-store');
    return;
  }

  console.log(`ApeWorkflow context stores (${payload.context_stores.length})`);
  console.log('');
...
export function registerContextStoreCommand(program: Command): void {
  ...
  contextStore
    ...
    .option('--path <path>', 'Context store folder path; defaults to ApeWorkflow managed local data')
