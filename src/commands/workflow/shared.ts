...
/**
 * Returns the list of available change directory names under apeworkflow/changes/.
...
export async function validateChangeExists(
  ...
  if (!changeName) {
    ...
    if (available.length === 0) {
      throw new Error('No changes found. Create one with: apeworkflow new change <name>');
    }
    throw new Error(
  ...
  if (!exists) {
    ...
    if (available.length === 0) {
      throw new Error(
        `Change '${changeName}' not found. No changes exist. Create one with: apeworkflow new change <name>`
      );
    }
    throw new Error(
