...
import { APEWORKFLOW_DIR_NAME } from '../core/config.js';
...
const WORKFLOW_PROMPT_META: Record<string, WorkflowPromptMeta> = {
  ...
  onboard: {
    name: 'Onboard',
    description: 'Guided onboarding flow for ApeWorkflow',
  },
};
...
function maybeWarnProjectConfigDrift(
  ...
  const apeworkflowDir = path.join(projectDir, APEWORKFLOW_DIR_NAME);
  if (!fs.existsSync(apeworkflowDir)) {
    return;
  }
  if (!hasProjectConfigDrift(projectDir, state.workflows, state.delivery)) {
    return;
  }
  console.log(colorize('Warning: Global config is not applied to this project. Run `apeworkflow update` to sync.'));
}
...
async function maybeWarnConfigDrift(
  ...
  if (workspaceContext) {
    ...
    if (hasWorkspaceSkillProfileDrift(viewState)) {
      console.log(
        colorize(
          'Warning: Workspace-local agent skills are out of sync with the active global profile. Run `apeworkflow workspace update` to sync.'
        )
      );
    }
    return;
...
function printConfigProfileApplyGuidance(workspaceContext: WorkspaceConfigProfileContext | null): void {
  if (workspaceContext) {
    console.log('Config updated. Run `apeworkflow workspace update` to apply it to workspace-local skills.');
    return;
  }

  console.log('Config updated. Run `apeworkflow update` in your projects to apply.');
}
...
export function registerConfigCommand(program: Command): void {
  const configCmd = program
    .command('config')
    .description('View and modify global ApeWorkflow configuration')
  ...
  configCmd
    ...
    .action((key: string, value: string, options: { string?: boolean; allowUnknown?: boolean }) => {
      ...
      if (!keyValidation.valid && !allowUnknown) {
        ...
        console.error('Use "apeworkflow config list" to see available keys.');
  ...
  configCmd
    ...
    .action(async (options: { all?: boolean; yes?: boolean }) => {
      if (!options.all) {
        ...
        console.error('Usage: apeworkflow config reset --all [-y]');
        process.exitCode = 1;
        return;
      }
  ...
  configCmd
    ...
    .action(async (preset?: string) => {
      // Preset shortcut: `apeworkflow config profile core`
      if (preset === 'core') {
        const config = getGlobalConfig();
        config.profile = 'core';
      ...
      if (!process.stdout.isTTY) {
        console.error('Interactive mode required. Use `apeworkflow config profile core` or set config via environment/flags.');
        process.exitCode = 1;
        return;
      }
      ...
      try {
        ...
        if (workspaceContext) {
          ...
          if (applyNow) {
            try {
              execSync('npx apeworkflow workspace update', {
                stdio: 'inherit',
                cwd: workspaceContext.commandCwd,
              });
              console.log('Run `apeworkflow workspace update` in your other workspaces to apply.');
            } catch {
              console.error('`apeworkflow workspace update` failed. Please run it manually to apply the profile changes.');
              process.exitCode = 1;
            }
            return;
          }
        ...
        // Check if inside an ApeWorkflow project
        ...
        if (fs.existsSync(apeworkflowDir)) {
          ...
          if (applyNow) {
            try {
              execSync('npx apeworkflow update', { stdio: 'inherit', cwd: projectDir });
              console.log('Run `apeworkflow update` in your other projects to apply.');
            } catch {
              console.error('`apeworkflow update` failed. Please run it manually to apply the profile changes.');
              process.exitCode = 1;
            }
            return;
          }
