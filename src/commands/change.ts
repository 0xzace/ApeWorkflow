  ...
  async show(changeName?: string, options?: { json?: boolean; requirementsOnly?: boolean; deltasOnly?: boolean; noInteractive?: boolean }): Promise<void> {
    ...
    if (!changeName) {
      ...
      if (canPrompt && changes.length > 0) {
        ...
        console.error('Hint: use "apeworkflow change list" to view available changes.');
        process.exitCode = 1;
        return;
      }
    }
    ...
    if (options?.json) {
      const changeDetails = await Promise.all(
        changes.map(async (changeName) => {
          ...
          const taskStatus = await getTaskProgressForChange(changesPath, changeName);
      ...
      for (const changeName of sorted) {
        ...
        try {
          ...
          const taskStatusText = ` [tasks ${completed}/${total}]`;
          const changeDir = path.join(changesPath, changeName);
