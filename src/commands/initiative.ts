...
function requireNonBlankOption(
  ...
  if (value === undefined || value.trim().length === 0) {
    throw new InitiativeCliError(`Pass --${flagName} <value>.`, code, {
      target,
      fix: `apeworkflow initiative create <id> --${flagName} <value>`,
    });
  }
...
function requireInitiativeId(
  ...
  if (id === undefined || id.trim().length === 0) {
    throw new InitiativeCliError('Pass an initiative id.', 'initiative_id_required', {
      ...
      fix: `apeworkflow initiative ${commandName} <id>`,
    });
  }
...
function printCreateHuman(payload: InitiativeCreateOutput): void {
  ...
  console.log(`  apeworkflow initiative list ${formatContextStoreSelector(payload.context_store)}`);
}
...
function printListHuman(payload: InitiativeListOutput): void {
  if (payload.context_store) {
    console.log(`ApeWorkflow initiatives in ${payload.context_store.id} (${payload.initiatives.length})`);
  ...
  console.log(
    `ApeWorkflow initiatives (${payload.initiatives.length} across ${payload.context_stores.length} stores)`
  );
  console.log('');
...
function printShowHuman(payload: InitiativeShowOutput): void {
  ...
  console.log(`ApeWorkflow initiative: ${payload.initiative.title}`);
  console.log('');
  console.log(`ID: ${payload.initiative.id}`);
