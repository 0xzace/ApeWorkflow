...
function workspaceNotInKnownViewsWarning(): WorkspaceStatus {
  return makeStatus(
    ...
    'This workspace is not in the managed local workspace views list.',
    {
      ...
      fix: 'Use apeworkflow workspace list to inspect managed workspace views.',
    }
  );
}
...
export async function selectWorkspaceForCommand(
  ...
  if (options.workspace) {
    ...
    if (!entry) {
      throw new WorkspaceCliError(
        `Unknown ApeWorkflow workspace '${workspaceName}'.`,
        'workspace_not_found',
        {
          target: 'workspace.name',
          fix: 'Run apeworkflow workspace list to see known workspaces.',
        }
      );
    }
  ...
  if (options.json || resolveNoInteractive(options) || !isInteractive(options)) {
    ...
    const fix = usesPositionalName
      ? `apeworkflow workspace ${commandName} <name>`
      : `apeworkflow workspace ${commandName} --workspace <name>`;

    throw new WorkspaceCliError(
      usesPositionalName
        ? `Multiple ApeWorkflow workspaces are known. Known workspaces: ${knownNames}. Pass a workspace name.`
        : `Multiple ApeWorkflow workspaces are known. Known workspaces: ${knownNames}. Pass --workspace <name>.`,
  ...
  if (!selectedEntry) {
    throw new WorkspaceCliError(
      `Unknown ApeWorkflow workspace '${selectedName}'.`,
      'workspace_not_found',
      {
        target: 'workspace.name',
        fix: 'Run apeworkflow workspace list to see known workspaces.',
