...
function formatDuplicateLinkMessage(
  ...
  return [
    ...
    `  apeworkflow workspace link archived-${linkName} ${replacementPath}`,
    '',
    'If you meant to change the existing link path:',
    `  apeworkflow workspace relink ${linkName} ${replacementPath}`,
  ].join('\n');
}
...
function duplicateLinkError(
  ...
  return new WorkspaceCliError(
    ...
    'duplicate_link_name',
    {
      ...
      fix: `Choose a different link name or run 'apeworkflow workspace relink ${linkName} ${replacementPath}'.`,
    }
  );
}
...
function localStateInvalidStatus(error: unknown): WorkspaceStatus {
  return makeStatus(
    ...
    `Machine-local paths could not be read: ${asErrorMessage(error)}`,
    {
      ...
      fix: 'Repair .apeworkflow-workspace/view.yaml, then run apeworkflow workspace relink <name> <path> for affected links.',
    }
  );
}
...
function workspaceSkillDriftStatus(workspaceName: string): WorkspaceStatus {
  return makeStatus(
    ...
    'Workspace-local agent skills are out of sync with the active global profile.',
    {
      ...
      fix: `apeworkflow workspace update --workspace ${workspaceName}`,
    }
  );
}
      ...
      workspace: {
        ...
        status: [
          makeStatus(
            ...
            {
              target: 'workspace.root',
              fix: 'Repair .apeworkflow-workspace/view.yaml before using this workspace.',
            }
          ),
        ],
      },
  ...
  for (const linkName of linkNames) {
      ...
      linkStatus.push(
        makeStatus(
          ...
          'Shared link does not have a local path on this machine.',
          {
            ...
            fix: `apeworkflow workspace relink ${linkName} /path/to/${linkName}`,
          }
        )
      );
    }
    ...
    if (localPath) {
      if (await directoryExists(localPath)) {
        const candidateSpecsPath = path.join(localPath, 'apeworkflow', 'specs');
...
async function readWorkspaceViewForMutation(selected: SelectedWorkspace): Promise<WorkspaceViewState> {
  if (!(await directoryExists(selected.root)) || !(await isWorkspaceRoot(selected.root))) {
    throw new WorkspaceCliError(
      ...
      {
        target: 'workspace.root',
        fix: 'Run apeworkflow workspace list to inspect known workspaces.',
      }
    );
  }
...
export async function updateWorkspaceLink(
  ...
  if (!hasWorkspaceLink(viewState.links, linkName)) {
    throw new WorkspaceCliError(`Unknown workspace link '${linkName}'.`, 'unknown_link_name', {
      target: `links.${linkName}`,
      fix: 'Run apeworkflow workspace doctor to see linked repos or folders.',
    });
  }
...
export async function selectOrCreateWorkspaceForInitiativeOpen(input: {
  ...
  if (existingDerived) {
    ...
    throw new WorkspaceCliError(
      ...
      {
        target: 'workspace.name',
        fix: `Retry with an explicit workspace name: apeworkflow workspace open <name> --initiative ${getWorkspaceContextStoreId(input.context)}/${getWorkspaceContextInitiativeId(input.context)}`,
      }
    );
  }
