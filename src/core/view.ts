...
export class ViewCommand {
  async execute(targetPath: string = '.'): Promise<void> {
    const apeworkflowDir = path.join(targetPath, 'apeworkflow');
    
    if (!fs.existsSync(apeworkflowDir)) {
      console.error(chalk.red('No apeworkflow directory found'));
      process.exit(1);
    }

    console.log(chalk.bold('\nApeWorkflow Dashboard\n'));
    console.log('═'.repeat(60));

    // Get changes and specs data
    const changesData = await this.getChangesData(apeworkflowDir);
    const specsData = await this.getSpecsData(apeworkflowDir);
    ...
    console.log('\n' + '═'.repeat(60));
    console.log(chalk.dim(`\nUse ${chalk.white('apeworkflow list --changes')} or ${chalk.white('apeworkflow list --specs')} for detailed views`));
  }

  private async getChangesData(apeworkflowDir: string): Promise<{
    draft: Array<{ name: string }>;
    active: Array<{ name: string; progress: { total: number; completed: number } }>;
    completed: Array<{ name: string }>;
  }> {
    const changesDir = path.join(apeworkflowDir, 'changes');
  ...
  private async getSpecsData(apeworkflowDir: string): Promise<Array<{ name: string; requirementCount: number }>> {
    const specsDir = path.join(apeworkflowDir, 'specs');
