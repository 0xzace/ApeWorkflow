...
export class PowerShellInstaller {
  ...
  getInstallationPath(): string {
    ...
    return path.join(profileDir, 'ApeWorkflowCompletion.ps1');
  }
  ...
  async configureProfile(scriptPath: string): Promise<boolean> {
    ...
    for (const profilePath of profilePaths) {
      try {
        ...
        // Add ApeWorkflow completion configuration with markers
        const apeworkflowBlock = [
          '',
        ...
        const newContent = profileContent + apeworkflowBlock;
  ...
  async removeProfileConfig(): Promise<boolean> {
    ...
    for (const profilePath of profilePaths) {
      try {
        ...
        // Remove APEWORKFLOW:START -> APEWORKFLOW:END block
        const startMarker = '# APEWORKFLOW:START';
        const endMarker = '# APEWORKFLOW:END';
        const startIndex = profileContent.indexOf(startMarker);

        if (startIndex === -1) {
          continue; // No ApeWorkflow block found
        }
  ...
  private generateInstructions(installedPath: string): string[] {
    ...
    return [
      ...
      '',
      '  # Source ApeWorkflow completions',
