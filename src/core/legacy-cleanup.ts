/**
 * Legacy cleanup module for detecting and removing ApeWorkflow artifacts
...
import { APEWORKFLOW_MARKERS } from './config.js';

/**
 * Legacy config file names from the old ToolRegistry.
 * These were config files created at project root with ApeWorkflow markers.
 */
export const LEGACY_CONFIG_FILES = [
  ...
  'QODER.md',
  'IFLOW.md',
  'AGENTS.md', // root AGENTS.md (not apeworkflow/AGENTS.md)
  'QWEN.md',
] as const;
...
export const LEGACY_SLASH_COMMAND_PATHS: Record<string, LegacySlashCommandPattern> = {
  // Directory-based: .tooldir/commands/apeworkflow/ or .tooldir/commands/apeworkflow/*.md
  'claude': { type: 'directory', path: '.claude/commands/apeworkflow' },
  'codebuddy': { type: 'directory', path: '.codebuddy/commands/apeworkflow' },
  'qoder': { type: 'directory', path: '.qoder/commands/apeworkflow' },
  'lingma': { type: 'directory', path: '.lingma/commands/apeworkflow' },
  'crush': { type: 'directory', path: '.crush/commands/apeworkflow' },
  'gemini': { type: 'directory', path: '.gemini/commands/apeworkflow' },
  'costrict': { type: 'directory', path: '.cospec/apeworkflow/commands' },

  // File-based: individual apeworkflow-*.md files in a commands/workflows/prompts folder
  'cursor': { type: 'files', pattern: '.cursor/commands/apeworkflow-*.md' },
  'windsurf': { type: 'files', pattern: '.windsurf/workflows/apeworkflow-*.md' },
  'kilocode': { type: 'files', pattern: '.kilocode/workflows/apeworkflow-*.md' },
  'kiro': { type: 'files', pattern: '.kiro/prompts/apeworkflow-*.prompt.md' },
  'github-copilot': { type: 'files', pattern: '.github/prompts/apeworkflow-*.prompt.md' },
  'amazon-q': { type: 'files', pattern: '.amazonq/prompts/apeworkflow-*.md' },
  'cline': { type: 'files', pattern: '.clinerules/workflows/apeworkflow-*.md' },
  'roocode': { type: 'files', pattern: '.roo/commands/apeworkflow-*.md' },
  'auggie': { type: 'files', pattern: '.augment/commands/apeworkflow-*.md' },
  'factory': { type: 'files', pattern: '.factory/commands/apeworkflow-*.md' },
  'opencode': { type: 'files', pattern: ['.opencode/command/ape-*.md', '.opencode/command/apeworkflow-*.md'] },
  'continue': { type: 'files', pattern: '.continue/prompts/apeworkflow-*.prompt' },
  'antigravity': { type: 'files', pattern: '.agent/workflows/apeworkflow-*.md' },
  'iflow': { type: 'files', pattern: '.iflow/commands/apeworkflow-*.md' },
  'junie': { type: 'files', pattern: ['.junie/commands/ape-*.md', '.junie/commands/apeworkflow-*.md'] },
  'qwen': { type: 'files', pattern: '.qwen/commands/apeworkflow-*.toml' },
  'codex': { type: 'files', pattern: '.codex/prompts/apeworkflow-*.md' },
};
...
export interface LegacyDetectionResult {
  /** Config files with ApeWorkflow markers detected */
  configFiles: string[];
  ...
  /** Whether apeworkflow/AGENTS.md exists */
  hasApeAgents: boolean;
  /** Whether apeworkflow/project.md exists (preserved, migration hint only) */
  hasProjectMd: boolean;
  /** Whether root AGENTS.md has ApeWorkflow markers */
...
/**
 * Detects all legacy ApeWorkflow artifacts in a project.
 *
...
export async function detectLegacyArtifacts(
  ...
  result.hasApeAgents = structureResult.hasApeAgents;
...
/**
 * Detects legacy config files with ApeWorkflow markers.
...
export async function detectLegacyConfigFiles(
  ...
  for (const fileName of LEGACY_CONFIG_FILES) {
    ...
    if (await FileSystemUtils.fileExists(filePath)) {
      ...
      if (hasApeWorkflowMarkers(content)) {
 ...
 *
 * @param projectPath - The root path of the project
 * @param pattern - Glob pattern like '.cursor/commands/apeworkflow-*.md'
...
async function findLegacySlashCommandFiles(
  ...
  try {
    ...
    // apeworkflow-*.md -> /^apeworkflow-.*\.md$/
    // apeworkflow-*.prompt.md -> /^apeworkflow-.*\.prompt\.md$/
    // apeworkflow-*.toml -> /^apeworkflow-.*\.toml$/
...
/**
 * Detects legacy ApeWorkflow structure files (AGENTS.md and project.md).
 *
 * @param projectPath - The root path of the project
...
export async function detectLegacyStructureFiles(
  ...
  let hasApeAgents = false;
  let hasProjectMd = false;
  let hasRootAgentsWithMarkers = false;

  // Check for apeworkflow/AGENTS.md
  const apeworkflowAgentsPath = FileSystemUtils.joinPath(projectPath, 'apeworkflow', 'AGENTS.md');
  hasApeAgents = await FileSystemUtils.fileExists(apeworkflowAgentsPath);

  // Check for apeworkflow/project.md (for migration messaging, not deleted)
  const projectMdPath = FileSystemUtils.joinPath(projectPath, 'apeworkflow', 'project.md');
  hasProjectMd = await FileSystemUtils.fileExists(projectMdPath);

  // Check for root AGENTS.md with ApeWorkflow markers
  ...
  if (await FileSystemUtils.fileExists(rootAgentsPath)) {
    ...
    hasRootAgentsWithMarkers = hasApeWorkflowMarkers(content);
  }

  return { hasApeAgents, hasProjectMd, hasRootAgentsWithMarkers };
}

/**
 * Checks if content contains ApeWorkflow markers.
 *
 * @param content - File content to check
 * @returns True if both start and end markers are present
 */
export function hasApeWorkflowMarkers(content: string): boolean {
  return (
    content.includes(APEWORKFLOW_MARKERS.start) && content.includes(APEWORKFLOW_MARKERS.end)
  );
}

/**
 * Checks if file content is 100% ApeWorkflow content (only markers and whitespace outside).
 *
 * @param content - File content to check
 * @returns True if content outside markers is only whitespace
 */
export function isOnlyApeWorkflowContent(content: string): boolean {
  const startIndex = content.indexOf(APEWORKFLOW_MARKERS.start);
  const endIndex = content.indexOf(APEWORKFLOW_MARKERS.end);
  ...
  const after = content.substring(endIndex + APEWORKFLOW_MARKERS.end.length);

  return before.trim() === '' && after.trim() === '';
}

/**
 * Removes the ApeWorkflow marker block from file content.
 ...
 *
 * @param content - File content with ApeWorkflow markers
 * @returns Content with marker block removed
 */
export function removeMarkerBlock(content: string): string {
  return removeMarkerBlockUtil(content, APEWORKFLOW_MARKERS.start, APEWORKFLOW_MARKERS.end);
}
...
/**
 * Cleans up legacy ApeWorkflow artifacts from a project.
 * Preserves apeworkflow/project.md (shows migration hint instead of deleting).
 *
 * @param projectPath - The root path of the project
...
export async function cleanupLegacyArtifacts(
  ...
  // Delete legacy slash command directories (these are 100% ApeWorkflow-managed)
  ...
  // Delete legacy slash command files (these are 100% ApeWorkflow-managed)
  ...
  // Delete apeworkflow/AGENTS.md (this is inside apeworkflow/, it's ApeWorkflow-managed)
  if (detection.hasApeAgents) {
    const agentsPath = FileSystemUtils.joinPath(projectPath, 'apeworkflow', 'AGENTS.md');
    if (await FileSystemUtils.fileExists(agentsPath)) {
      try {
        await fs.unlink(agentsPath);
        result.deletedFiles.push('apeworkflow/AGENTS.md');
      } catch (error: any) {
        result.errors.push(`Failed to delete apeworkflow/AGENTS.md: ${error.message}`);
      }
    }
  }

  // Handle root AGENTS.md with ApeWorkflow markers - remove markers only, NEVER delete
...
export function formatCleanupSummary(result: CleanupResult): string {
  ...
  if (result.deletedFiles.length > 0 || result.deletedDirs.length > 0 || result.modifiedFiles.length > 0) {
    ...
    for (const dir of result.deletedDirs) {
      lines.push(`  ✓ Removed ${dir}/ (replaced by /ape:*)`);
    }

    for (const file of result.modifiedFiles) {
      lines.push(`  ✓ Removed ApeWorkflow markers from ${file}`);
    }
  }
 ...
 * Only includes ApeWorkflow-managed files (slash commands, apeworkflow/AGENTS.md).
...
function buildRemovalsList(detection: LegacyDetectionResult): Array<{ path: string; explanation: string }> {
  ...
  // Slash command directories (these are 100% ApeWorkflow-managed)
  ...
  // Slash command files (these are 100% ApeWorkflow-managed)
  for (const file of detection.slashCommandFiles) {
    removals.push({ path: file, explanation: 'replaced by skills/' });
  }

  // apeworkflow/AGENTS.md (inside apeworkflow/, it's ApeWorkflow-managed)
  if (detection.hasApeAgents) {
    removals.push({ path: 'apeworkflow/AGENTS.md', explanation: 'obsolete workflow file' });
  }
...
function buildUpdatesList(detection: LegacyDetectionResult): Array<{ path: string; explanation: string }> {
  ...
  for (const file of detection.configFilesToUpdate) {
    updates.push({ path: file, explanation: 'removing ApeWorkflow markers' });
  }
...
export function formatDetectionSummary(detection: LegacyDetectionResult): string {
  ...
  lines.push(chalk.bold('Upgrading to the new ApeWorkflow'));
  lines.push('');
  lines.push('ApeWorkflow now uses agent skills, the emerging standard across coding');
  ...
  if (updates.length > 0) {
    ...
    lines.push(chalk.dim('ApeWorkflow markers will be removed, your content preserved:'));
  ...
  for (const file of detection.slashCommandFiles) {
    ...
    for (const [toolId, pattern] of Object.entries(LEGACY_SLASH_COMMAND_PATHS)) {
      if (pattern.type === 'files' && pattern.pattern) {
        ...
        // e.g., '.cursor/commands/apeworkflow-*.md' -> /^\.cursor\/commands\/apeworkflow-.*\.md$/
        const patterns = Array.isArray(pattern.pattern) ? pattern.pattern : [pattern.pattern];
        let matched = false;
        for (const p of patterns) {
...
export function formatProjectMdMigrationHint(): string {
  ...
  lines.push('  • apeworkflow/project.md');
  lines.push(chalk.dim('    We won\'t delete this file. It may contain useful project context.'));
  lines.push('');
  lines.push(chalk.dim('    The new apeworkflow/config.yaml has a "context:" section for planning'));
  lines.push(chalk.dim('    context. This is included in every ApeWorkflow request and works more'));
