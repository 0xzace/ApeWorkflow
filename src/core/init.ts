 ...
 * Init Command
 *
 * Sets up ApeWorkflow with Agent Skills and /ape:* slash commands.
...
export class InitCommand {
  ...
  async execute(targetPath: string): Promise<void> {
    ...
    const apeworkflowDir = APEWORKFLOW_DIR_NAME;
    const apeworkflowPath = path.join(projectPath, apeworkflowDir);

    // Validation happens silently in the background
    const extendMode = await this.validate(projectPath, apeworkflowPath);
    ...
    await this.createDirectoryStructure(apeworkflowPath, extendMode);
  ...
  private async validate(
    projectPath: string,
    apeworkflowPath: string
  ): Promise<boolean> {
    const extendMode = await FileSystemUtils.directoryExists(apeworkflowPath);
  ...
  private async handleLegacyCleanup(projectPath: string, extendMode: boolean): Promise<void> {
    ...
    if (this.force || !canPrompt) {
      ...
      // Legacy slash commands are 100% ApeWorkflow-managed, and config file cleanup
  ...
  private async getSelectedTools(
    ...
    if (configuredNames.length > 0) {
      console.log(`ApeWorkflow configured: ${configuredNames.join(', ')} (pre-selected)`);
    }
  ...
  private async createDirectoryStructure(apeworkflowPath: string, extendMode: boolean): Promise<void> {
    if (extendMode) {
      // In extend mode, just ensure directories exist without spinner
      const directories = [
        apeworkflowPath,
        path.join(apeworkflowPath, 'specs'),
        path.join(apeworkflowPath, 'changes'),
        path.join(apeworkflowPath, 'changes', 'archive'),
      ];
    ...
    const spinner = this.startSpinner('Creating ApeWorkflow structure...');
    ...
    spinner.stopAndPersist({
      ...
      text: PALETTE.white('ApeWorkflow structure created'),
    });
  }
  ...
  private async createConfig(apeworkflowPath: string, extendMode: boolean): Promise<'created' | 'exists' | 'skipped'> {
    const configPath = path.join(apeworkflowPath, 'config.yaml');
    const configYmlPath = path.join(apeworkflowPath, 'config.yml');
...
`;
  ...
  private displaySuccessMessage(
    ...
    console.log();
    console.log(chalk.bold('ApeWorkflow Setup Complete'));
    console.log();
    ...
    if (configStatus === 'created') {
      console.log(`Config: apeworkflow/config.yaml (schema: ${DEFAULT_SCHEMA})`);
    } else if (configStatus === 'exists') {
      // Show actual filename (config.yaml or config.yml)
      const configYaml = path.join(projectPath, APEWORKFLOW_DIR_NAME, 'config.yaml');
      const configYml = path.join(projectPath, APEWORKFLOW_DIR_NAME, 'config.yml');
      const configName = fs.existsSync(configYaml) ? 'config.yaml' : fs.existsSync(configYml) ? 'config.yml' : 'config.yaml';
      console.log(`Config: apeworkflow/${configName} (exists)`);
    } else {
    ...
    if (activeWorkflows.includes('propose')) {
      ...
      console.log('  Start your first change: /ape:propose "your idea"');
    } else if (activeWorkflows.includes('new')) {
      console.log(chalk.bold('Getting started:'));
      console.log('  Start your first change: /ape:new "your idea"');
    } else {
      console.log("Done. Run 'apeworkflow config profile' to configure your workflows.");
    }

    // Links
    console.log();
    console.log(`Learn more: ${chalk.cyan('https://github.com/0xzace/ApeWorkflow')}`);
    console.log(`Feedback:   ${chalk.cyan('https://github.com/0xzace/ApeWorkflow/issues')}`);
