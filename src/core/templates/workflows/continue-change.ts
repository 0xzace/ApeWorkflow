...
1. **If no change name provided, prompt for selection**
   ...
   Run \`apeworkflow list --json\` to get available changes sorted by most recently modified. Then use the **AskUserQuestion tool** to let the user select which change to work on.
...
- **tasks.md**: Break down the implementation into a plain task outline for brainstorming and plan generation (outline only).
...
export function getApeContinueCommandTemplate(): CommandTemplate {
  return {
    name: 'APE: Continue',
