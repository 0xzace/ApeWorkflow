...
2. **Determine the workflow schema**
   ...
   **Use a different schema only if the user mentions:**
   ...
   - "show workflows" or "what workflows" → run \`apeworkflow schemas --json\` and let them choose
...
5. **Get instructions for the first artifact**
   ...
   \`\`\`bash
   apeworkflow instructions <first-artifact-id> --change "<name>"
   \`\`\`
...
export function getApeNewCommandTemplate(): CommandTemplate {
  return {
    name: 'APE: New',
    description: 'Start a new change using the experimental artifact workflow (APE)',
    category: 'Workflow',
