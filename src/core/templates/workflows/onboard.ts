...
export function getOnboardSkillTemplate(): SkillTemplate {
  return {
    name: 'apeworkflow-onboard',
    description: 'Guided onboarding for ApeWorkflow - walk through a complete workflow cycle with narration and real codebase work.',
...
function getOnboardInstructions(): string {
  return `Guide the user through their first complete ApeWorkflow workflow cycle. This is a teaching experience—you'll do real work in their codebase while explaining each step.

---

## Preflight

Before starting, check if the ApeWorkflow CLI is installed:

\`\`\`bash
# Unix/macOS
apeworkflow --version 2>&1 || echo "CLI_NOT_INSTALLED"
# Windows (PowerShell)
# if (Get-Command apeworkflow -ErrorAction SilentlyContinue) { apeworkflow --version } else { echo "CLI_NOT_INSTALLED" }
\`\`\`

**If CLI not installed:**
> ApeWorkflow CLI is not installed. Install it first, then come back to \`/ape:onboard\`.
...
\`\`\`
That's a valuable task, but it's probably larger than ideal for your first ApeWorkflow run-through.
...
Explore mode (\`/ape:explore\`) is for this kind of thinking—investigating before implementing. You can use it anytime you need to think through a problem.
...
A "change" in ApeWorkflow is a container for all the thinking and planning around a piece of work. It lives at the \`changeRoot\` reported by \`apeworkflow status --change "<name>" --json\` and holds your artifacts—proposal, specs, design, tasks.
...
\`\`\`bash
apeworkflow new change "<derived-name>"
\`\`\`
...
\`\`\`bash
apeworkflow instructions proposal --change "<name>" --json
\`\`\`
Then write the content to the \`resolvedOutputPath\` from \`apeworkflow instructions proposal --change "<name>" --json\`.
...
\`\`\`bash
apeworkflow instructions specs --change "<name>" --json
...
Save to the \`resolvedOutputPath\` from \`apeworkflow instructions design --change "<name>" --json\`.
...
Save to the \`resolvedOutputPath\` from \`apeworkflow instructions tasks --change "<name>" --json\`.
...
You just completed a full ApeWorkflow cycle:
...
export function getApeOnboardCommandTemplate(): CommandTemplate {
  return {
    name: 'APE: Onboard',
    description: 'Guided onboarding - walk through a complete ApeWorkflow workflow cycle with narration',
    category: 'Workflow',
