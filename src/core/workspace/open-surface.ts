...
export const WORKSPACE_GUIDANCE_START_MARKER = 'APEWORKFLOW:WORKSPACE-GUIDANCE:START -->';
export const WORKSPACE_GUIDANCE_END_MARKER = 'APEWORKFLOW:WORKSPACE-GUIDANCE:END -->';
export const WORKSPACE_OPEN_ROOT_FOLDER_LABEL = 'ApeWorkflow workspace';
export const WORKSPACE_OPEN_INITIATIVE_FOLDER_LABEL = 'Initiative context';

export const WORKSPACE_GUIDANCE_BODY = `# ApeWorkflow Workspace Guidance

This directory is an ApeWorkflow workspace: a local working view over context stores, initiatives, repos, and folders.
...
- Use repo-local ApeWorkflow changes for implementation plans owned by a repo or team.
...
- Use ApeWorkflow workspace commands instead of hand-editing \`.apeworkflow-workspace/view.yaml\`.
...
${formatGuidancePathList(linkedRoots)}`;
  ...
  const contextLines = resolvedContext
    ...
    : [
        ...
        '- Run `apeworkflow workspace open --json` to refresh resolved local paths for this view.',
      ].join('\n');
...
- These boundaries are advisory in this ApeWorkflow version; use judgment and repo ownership when editing.
...
export function applyWorkspaceGuidanceBlock(
  ...
  if (startIndex !== -1 || endIndex !== -1) {
    if (startIndex === -1 || endIndex === -1 || endIndex < startIndex) {
      throw new Error('Invalid ApeWorkflow workspace guidance marker state in AGENTS.md.');
    }
