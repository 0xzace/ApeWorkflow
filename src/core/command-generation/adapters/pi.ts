 ...
 * File path: .pi/prompts/ape-<id>.md
 * Frontmatter: description
 *
 * Pi uses the filename (minus .md) as the slash command name, so
 * ape-propose.md → /ape-propose. Command references in the body
 * are transformed from /ape: to /ape- for consistency.
 */
export const piAdapter: ToolCommandAdapter = {
  toolId: 'pi',

  getFilePath(commandId: string): string {
    return path.join('.pi', 'prompts', `ape-${commandId}.md`);
  },

  formatFile(content: CommandContent): string {
    // Transform /ape: references to /ape- and inject $@ for template args
