 ...
 * Update Command
 *
 * Refreshes ApeWorkflow skills and commands for configured tools.
...
export class UpdateCommand {
  ...
  async execute(projectPath: string): Promise<void> {
    ...
    const apeworkflowPath = path.join(resolvedProjectPath, APEWORKFLOW_DIR_NAME);

    // 1. Check apeworkflow directory exists
    if (!await FileSystemUtils.directoryExists(apeworkflowPath)) {
      throw new Error(`No ApeWorkflow directory found. Run 'apeworkflow init' first.`);
    }

    // 2. Perform one-time migration if needed before any legacy upgrade generation.
    // Use detected tool directories to preserve existing ape skills/commands.
    ...
    if (configuredTools.length === 0 && newlyConfiguredTools.length === 0) {
      ...
      console.log(chalk.dim('Run "apeworkflow init" to set up tools.'));
      return;
    }
    ...
    const toolStatuses = configuredTools.map((toolId) => {
      const status = getToolVersionStatus(resolvedProjectPath, toolId, APEWORKFLOW_VERSION);
    ...
    if (updatedTools.length > 0) {
      console.log(chalk.green(`✓ Updated: ${updatedTools.join(', ')} (v${APEWORKFLOW_VERSION})`));
    }
    ...
    if (newlyConfiguredTools.length > 0) {
      ...
      console.log('  /ape:new       Start a new change');
      console.log('  /ape:continue  Create the next artifact');
      console.log('  /ape:apply     Implement tasks');
      console.log();
  ...
  private displayUpdatePlan(
    ...
    const updates = toolsToUpdate.map((toolId) => {
      ...
      if (status?.needsUpdate) {
        ...
        return `${status.toolId} (${fromVersion} → ${APEWORKFLOW_VERSION})`;
      }
  ...
  private detectNewTools(projectPath: string, configuredTools: string[]): void {
    ...
    if (newTools.length > 0) {
      ...
      console.log(
        chalk.yellow(
          `Detected new ${toolNoun}: ${newToolNames.join(', ')}. Run 'apeworkflow init' to add ${pronoun}.`
        )
      );
    }
  }
  ...
  /**
   * Detect and handle legacy ApeWorkflow artifacts.
