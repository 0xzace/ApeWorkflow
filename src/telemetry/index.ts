 ...
 * - Opt-out via APEWORKFLOW_TELEMETRY=0 or DO_NOT_TRACK=1
...
const POSTHOG_HOST = 'https://edge.apeworkflow.dev';
...
export function isTelemetryEnabled(): boolean {
  ...
  if (process.env.APEWORKFLOW_TELEMETRY === '0') {
    return false;
  }
 ...
 * @param version - The ApeWorkflow version
 */
...
export async function maybeShowTelemetryNotice(): Promise<void> {
  ...
  try {
    ...
    // Display notice
    console.log(
      'Note: ApeWorkflow collects anonymous usage stats. Opt out: APEWORKFLOW_TELEMETRY=0'
    );
