...
This guide explains the core ideas behind ApeWorkflow and how they fit together. For practical usage, see [Getting Started](getting-started.md) and [Workflows](workflows.md).

## Philosophy

ApeWorkflow is built around four principles:
...
**Fluid not rigid.** Traditional spec systems lock you into phases: first you plan, then you implement, then you're done. ApeWorkflow is more flexible — you can create artifacts in any order that makes sense for your work.

**Iterative not waterfall.** Requirements change. Understanding deepens. What seemed like a good approach at the start might not hold up after you see the codebase. ApeWorkflow embraces this reality.

**Easy not complex.** Some spec frameworks require extensive setup, rigid formats, or heavyweight processes. ApeWorkflow stays out of your way. Initialize in seconds, start working immediately, customize only if you need to.

**Brownfield-first.** Most software work isn't building from scratch — it's modifying existing systems. ApeWorkflow's delta-based approach makes it easy to specify changes to existing behavior, not just describe new systems.

## The Big Picture

ApeWorkflow organizes your work into two main areas:
...
Repo-local ApeWorkflow projects are the right default when one repo owns the planning, implementation, and archive flow. Some work spans several repos or folders. For that case, an ApeWorkflow coordination workspace is a machine-local view that keeps linked paths, opener state, and agent setup together.
...
Repo-local ApeWorkflow state keeps the existing shape:
...
Root-level `workspace.yaml` files are not ApeWorkflow workspace state. Workspace state is namespaced under `.apeworkflow-workspace/`, so other tools can keep owning root-level files with the same name.

That distinction matters. The workspace folder is a local coordination surface for opening and inspecting linked repos or folders. Each repo's `apeworkflow/` directory remains the home for repo-owned specs, repo-local changes, and implementation planning. Users do not need to run repo-local `apeworkflow init` inside a workspace folder.
...
```yaml
# .apeworkflow-workspace/view.yaml
version: 1
name: platform
context: null
links:
...
When a workspace opens an initiative, `context` records the selected context-store binding and initiative id. Registry-selected stores stay portable by id; path-selected stores intentionally preserve the runtime-local path because `.apeworkflow-workspace/view.yaml` is private local state.
...
Linked paths can be full repos, folders inside a large monorepo, or other existing folders. They do not need repo-local `apeworkflow/` state before they can participate in workspace planning. Later implementation, verify, or archive workflows may require more repo readiness, but planning visibility starts with the link.
...
Managed workspaces live under the standard ApeWorkflow data directory:

```text
getGlobalDataDir()/workspaces
```

That means `$XDG_DATA_HOME/apeworkflow/workspaces` when `XDG_DATA_HOME` is set, `~/.local/share/apeworkflow/workspaces` on Unix-style fallback, and `%LOCALAPPDATA%\apeworkflow\workspaces` on native Windows fallback. Native Windows shells, PowerShell, and WSL2 each keep the path strings for the runtime running ApeWorkflow. This foundation does not translate between `D:\repo`, `/mnt/d/repo`, and UNC WSL paths.
...
Workspace visibility is not change commitment. Set up a workspace when ApeWorkflow should know which repos or folders are relevant; create a change later when you are ready to plan a feature, fix, project, or other piece of work.
...
apeworkflow workspace relink api-service /new/path/to/api

# Check what this machine can resolve
apeworkflow workspace doctor
apeworkflow workspace doctor --workspace platform
...
apeworkflow workspace open --initiative billing-launch --store-path /repos/platform-context
```

`workspace setup` always creates the workspace in the standard workspace location, records it in the local registry, shows the workspace location, and requires at least one linked repo or folder. Interactive setup asks for a preferred opener and can install ApeWorkflow skills for selected agents. Non-interactive setup stores one only when `--opener codex-cli`, `--opener claude`, `--opener github-copilot`, or `--opener editor` is provided.

Workspace skills are installed only in the workspace root. The active global profile selects which workflow skills are generated; `--tools` selects which agents receive them. Workspace setup and update do not create slash command files even when global delivery includes commands. Run `apeworkflow workspace update` to refresh workspace-local guidance and add, refresh, or remove managed workspace-local skill directories without editing linked repos or folders.

ApeWorkflow also maintains root workspace open files: an ApeWorkflow-managed guidance block in `AGENTS.md` and a machine-local `<workspace-name>.code-workspace` file for VS Code and GitHub Copilot-in-VS-Code opens. A managed workspace is not a repo, so ApeWorkflow does not create a default workspace `.gitignore` or a default workspace-level `changes/` directory.
...
`workspace link` and `workspace relink` record existing folders only; they do not create, copy, move, initialize, or edit the linked repo or folder. After a successful link or relink, ApeWorkflow refreshes the managed guidance and VS Code workspace file.

Workspace commands that need one workspace can run from anywhere with `--workspace <name>`. If you run them inside a workspace folder or subdirectory, ApeWorkflow uses that current workspace. If several known workspaces are available and you do not pass `--workspace <name>`, human commands show a picker; `--json` and `--no-interactive` fail with a structured status error instead of prompting.
...
ApeWorkflow aims to avoid bureaucracy. Use the lightest level that still makes the change verifiable.
...
```
apeworkflow/changes/add-dark-mode/
├── proposal.md           # Why and what
├── design.md             # How (technical approach)
├── tasks.md              # Implementation checklist
├── .apeworkflow.yaml        # Change metadata (optional)
└── specs/                # Delta specs
    └── ui/
...
Delta specs are the key concept that makes ApeWorkflow work for brownfield development. They describe **what's changing** rather than restating the entire spec.
...
```yaml
# apeworkflow/schemas/spec-driven/schema.yaml
name: spec-driven
artifacts:
  - id: proposal
...
apeworkflow schema fork spec-driven research-first
```

**Example custom schema:**

```yaml
# apeworkflow/schemas/research-first/schema.yaml
name: research-first
artifacts:
  - id: research
...
│                                                                              │
│   ┌────────────────┐                                                         │
│   │  1. START      │  /ape:propose (core) or /ape:new (expanded)           │
│   │     CHANGE     │                                                         │
│   └───────┬────────┘                                                         │
│           │                                                                  │
│           ▼                                                                  │
│   ┌────────────────┐                                                         │
│   │  2. CREATE     │  /ape:ff or /ape:continue (expanded workflow)         │
...
| **Source of truth** | The `apeworkflow/specs/` directory, containing the current agreed-upon behavior |
