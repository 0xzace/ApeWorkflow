...
This guide explains how ApeWorkflow works after you've installed and initialized it. For installation instructions, see the [main README](../README.md#quick-start).

## How It Works

ApeWorkflow helps you and your AI coding assistant agree on what to build before any code is written.
...
```text
/ape:new ──► /ape:ff or /ape:continue ──► /ape:apply ──► /ape:verify ──► /ape:archive
```

The default global profile is `core`, which includes `propose`, `explore`, `apply`, `sync`, and `archive`. You can enable the expanded workflow commands with `apeworkflow config profile` and then `apeworkflow update`.

## What ApeWorkflow Creates

After running `apeworkflow init`, your project has this structure:
...
Delta specs are the key concept in ApeWorkflow. They show what's changing relative to your current specs.
...
The change folder moves to `apeworkflow/changes/archive/` for audit history.
...
If you've enabled the expanded workflow profile, you can also do this as two steps: `/ape:new` then `/ape:ff` (or `/ape:continue` incrementally).
...
AI:  Archiving add-dark-mode...
     ✓ Merged specs into apeworkflow/specs/ui/spec.md
...
- [Customization](customization.md) - Make ApeWorkflow work your way
