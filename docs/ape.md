...
APE is now the standard workflow for ApeWorkflow.

It's a **fluid, iterative workflow** for ApeWorkflow changes. No more rigid phases — just actions you can take anytime.

## Why This Exists

The legacy ApeWorkflow workflow works, but it's **locked down**:
...
**APE opens it up.** Now anyone can:
...
- **ApeWorkflow contributors** — experiment with new approaches without releases

We're all still learning what works best. APE lets us learn together.
...
```bash
# Make sure you have apeworkflow installed — skills are automatically generated
apeworkflow init
```

This creates skills in `.claude/skills/` (or equivalent) that AI coding assistants auto-detect.

By default, ApeWorkflow uses the `core` workflow profile (`propose`, `explore`, `apply`, `sync`, `archive`). If you want the expanded workflow commands (`new`, `continue`, `ff`, `verify`, `bulk-archive`, `onboard`), configure them with `apeworkflow config profile` and apply with `apeworkflow update`.

During setup, you'll be prompted to create a **project config** (`apeworkflow/config.yaml`). This is optional but recommended.
...
Config is created during `apeworkflow init`, or manually:

```yaml
# apeworkflow/config.yaml
schema: spec-driven
...
2. Change metadata (`.apeworkflow.yaml` in change directory)
...
- Run `apeworkflow schemas --json` to see artifact IDs for each schema

**Config not being applied:**

- Ensure file is at `apeworkflow/config.yaml` (not `.yml`)
...
| ------------------- | -------------------------------------------------------------------------------- |
| `/ape:propose`      | Create a change and generate planning artifacts in one step (default quick path) |
| `/ape:explore`      | Think through ideas, investigate problems, clarify requirements                  |
| `/ape:new`          | Start a new change scaffold (expanded workflow)                                  |
| `/ape:continue`     | Create the next artifact (expanded workflow)                                     |
| `/ape:ff`           | Fast-forward planning artifacts (expanded workflow)                              |
| `/ape:apply`        | Implement tasks, updating artifacts as needed                                    |
| `/ape:verify`       | Validate implementation against artifacts (expanded workflow)                    |
| `/ape:sync`         | Sync delta specs to main (default workflow, optional)                            |
| `/ape:archive`      | Archive when done                                                                |
| `/ape:bulk-archive` | Archive multiple completed changes (expanded workflow)                           |
| `/ape:onboard`      | Guided walkthrough of an end-to-end change (expanded workflow)                   |
...
Think through ideas, investigate problems, compare options. No structure required - just a thinking partner. When insights crystallize, transition to `/ape:propose` (default) or `/ape:new`/`/ape:ff` (expanded).
...
```text
/ape:new        # scaffold only
/ape:continue   # create one artifact at a time
/ape:ff         # create all planning artifacts at once
```
...
Works through tasks, checking them off as you go. If you're juggling multiple changes, you can run `/ape:apply <name>`; otherwise it should infer from the conversation and prompt you to choose if it can't tell.

### Finish up

```
/ape:archive   # Move to archive when done (prompts to sync specs if needed)
```
...
|                   | Legacy (`/apeworkflow:proposal`)          | APE (`/ape:*`)                            |
| ----------------- | ----------------------------------------- | ----------------------------------------- |
...
**The key insight:** work isn't linear. APE stops pretending it is.

## Architecture Deep Dive

This section explains how APE works under the hood and how it compares to the legacy workflow.
...
│         │                     │                     │                       │
│         ▼                     ▼                     ▼                       │
│   /apeworkflow:proposal   /apeworkflow:apply      /apeworkflow:archive              │
│                                                                             │
...
│                    │                                                        │
│                    ▼                                                        │
│   Generated Command Files (.claude/commands/apeworkflow/*.md)                  │
│                                                                             │
...
**APE** uses external schemas and a dependency graph engine:
...
│                    │                                                        │
│                    ▼                                                        │
│   Skill Files (.claude/skills/apeworkflow-*/SKILL.md)                          │
│                                                                             │
...
**APE** — agent queries for rich context:
...
```
  ...
  │  ┌────────────────────────────────────────────────────────────────────┐  │
  │  │  $ apeworkflow status --change "add-auth" --json                      │  │
  │  │                                                                    │  │
  │  │  {                                                                 │  │
  │  │    "artifacts": [                                                  │  │
  ...
  │  ┌────────────────────────────────────────────────────────────────────┐  │
  │  │  $ apeworkflow instructions specs --change "add-auth" --json          │  │
  │  │                                                                    │  │
  │  │  {                                                                 │  │
...
**APE** — natural iteration:

```
  /ape:new ───► /ape:continue ───► /ape:apply ───► /ape:archive
      │                │                  │
      │                │                  ├── "The design is wrong"
      │                │                  │
      │                │                  ▼
...
apeworkflow schema which my-workflow
```

Schemas are stored in `apeworkflow/schemas/` (project-local, version controlled) or `~/.local/share/apeworkflow/schemas/` (user global).

**Schema structure:**

```
apeworkflow/schemas/research-first/
├── schema.yaml
└── templates/
    ├── research.md
    ├── proposal.md
...
- Use `/ape:explore` to think through an idea before committing to a change
- `/ape:ff` when you know what you want, `/ape:continue` when exploring
- During `/ape:apply`, if something's wrong — fix the artifact, then continue
- Tasks track progress via checkboxes in `tasks.md`
- Check status anytime: `apeworkflow status --change "name"`

## Feedback

This is rough. That's intentional — we're learning what works.

Found a bug? Have ideas? Join us on [Discord](https://discord.gg/YctCnvvshC) or open an issue on [GitHub](https://github.com/0xzace/apeworkflow/issues).
