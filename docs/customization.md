...
ApeWorkflow provides three levels of customization:
...
The `apeworkflow/config.yaml` file is the easiest way to customize ApeWorkflow for your team. It lets you:
...
```bash
# Without config
apeworkflow new change my-feature --schema spec-driven

# With config - schema is automatic
apeworkflow new change my-feature
```
...
When ApeWorkflow needs a schema, it checks in this order:

1. CLI flag: `--schema <name>`
2. Change metadata (`.apeworkflow.yaml` in the change folder)
...
When project config isn't enough, create your own schema with a completely custom workflow. Custom schemas live in your project's `apeworkflow/schemas/` directory and are version-controlled with your code.
...
This copies the entire `spec-driven` schema to `apeworkflow/schemas/my-workflow/` where you can edit it freely.

**What you get:**

```text
apeworkflow/schemas/my-workflow/
...
```yaml
# apeworkflow/schemas/my-workflow/schema.yaml
name: my-workflow
version: 1
...
```bash
# Specify on command
apeworkflow new change feature --schema my-workflow
...
Schema: my-workflow
Source: project
Path: /path/to/project/apeworkflow/schemas/my-workflow
```

---

> **Note:** ApeWorkflow also supports user-level schemas at `~/.local/share/apeworkflow/schemas/` for sharing across projects, but project-level schemas in `apeworkflow/schemas/` are recommended since they're version-controlled with your code.
...
```yaml
# apeworkflow/schemas/rapid/schema.yaml
name: rapid
version: 1
...
```bash
apeworkflow schema fork spec-driven with-review
```
...
ApeWorkflow also supports community-maintained schemas distributed via standalone repositories. These provide opinionated workflows that integrate ApeWorkflow with other tools or systems, similar to how [github/spec-kit's community extension catalog](https://github.com/github/spec-kit/tree/main/extensions) works for spec-kit.

Community schemas are not vendored into ApeWorkflow core — they live in their own repositories with their own release cadence. To use one, copy the schema bundle into your project's `apeworkflow/schemas/<schema-name>/` directory (each repo's README has install instructions).
