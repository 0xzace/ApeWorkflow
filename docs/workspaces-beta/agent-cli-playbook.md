# ApeWorkflow CLI Playbook For Agents
...
apeworkflow initiative list --json
apeworkflow initiative show <store>/<initiative> --json
apeworkflow workspace doctor --json
```
...
Humans can run `apeworkflow context-store setup` and answer prompts. Agents should
pass the setup inputs explicitly.

```bash
apeworkflow context-store setup team-context --no-init-git --json
apeworkflow context-store setup team-context --path /path/to/team-context --init-git --json
```
...
```bash
apeworkflow initiative create billing-launch --store team-context --title "Billing Launch" --summary "Get billing live without losing the plot."
```
...
1. Resolve the workspace with `apeworkflow workspace doctor --json`.
2. Resolve the initiative with `apeworkflow initiative show <store>/<initiative> --json`.
...
```bash
apeworkflow new change add-billing-api --initiative team-context/billing-launch
```
...
```bash
apeworkflow workspace doctor --workspace billing-launch --json
apeworkflow context-store doctor --json
```
