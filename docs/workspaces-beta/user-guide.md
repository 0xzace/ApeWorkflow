...
ApeWorkflow asks for the context store name, where to put it, and whether to
...
> Create an ApeWorkflow initiative called `billing-launch` in `team-context`. Keep
> it short and useful.
...
Select the initiative from the picker. ApeWorkflow creates a local workspace view
...
when attached, and a small `ApeWorkflow workspace` folder last with `AGENTS.md`,
`.apeworkflow-workspace/view.yaml`, and the generated `.code-workspace` file.

Use `apeworkflow workspace open --initiative team-context/billing-launch --editor`
...
> Check this ApeWorkflow workspace. Resolve the selected initiative, list the
...
ApeWorkflow does not clone anything.
...
> ApeWorkflow artifacts should live.
...
> Create a draft repo-local ApeWorkflow proposal for the owning linked repo and
> link it to `team-context/billing-launch`. Resolve the workspace and initiative
> context yourself, run the needed ApeWorkflow commands from the correct repo, and
> report the files you created.

## Tiny Caveat Box

ApeWorkflow is not cloning, syncing, branching, or tracking progress dashboards in
