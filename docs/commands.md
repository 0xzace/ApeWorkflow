...
This is the reference for ApeWorkflow's slash commands. These commands are invoked in your AI coding assistant's chat interface (e.g., Claude Code, Cursor, Windsurf).
...
| `/ape:apply` | Implement tasks from the change |
...
| `/ape:continue` | Create the next artifact based on dependencies |
| `/ape:ff` | Fast-forward: create all planning artifacts at once |
| `/ape:verify` | Validate implementation matches artifacts |
| `/ape:bulk-archive` | Archive multiple changes at once |
| `/ape:onboard` | Guided tutorial through the complete workflow |

The default global profile is `core`. To enable expanded workflow commands, run `apeworkflow config profile`, select workflows, then run `apeworkflow update` in your project.
...
**Syntax:**
```text
/ape:propose [change-name-or-description]
```
...
AI:  Created apeworkflow/changes/add-dark-mode/
     ...
     ✓ design.md
     ✓ tasks.md
     Ready for implementation. Run /ape:apply.
```

**Tips:**
- Use this for the fastest end-to-end path
- If you want step-by-step artifact control, enable expanded workflows and use `/ape:new` + `/ape:continue`
...
- Can transition to `/ape:propose` (default) or `/ape:new` (expanded workflow) when insights crystallize
...
AI:  Ready when you are. Run /ape:propose add-jwt-auth to begin.
```
...
Start a new change scaffold. Creates the change folder and waits for you to generate artifacts with `/ape:continue` or `/ape:ff`.

This command is part of the expanded workflow set (not included in the default `core` profile).

**Syntax:**
```
/ape:new [change-name] [--schema <schema-name>]
```
...
**What it does:**
- Creates `apeworkflow/changes/<change-name>/` directory
- Creates `.apeworkflow.yaml` metadata file in the change folder
- Shows the first artifact template ready for creation
- Prompts for change name and schema if not provided

**What it creates:**
```
apeworkflow/changes/<change-name>/
└── .apeworkflow.yaml    # Change metadata (schema, created date)
```

**Example:**
```
You: /ape:new add-dark-mode

AI:  Created apeworkflow/changes/add-dark-mode/
     Schema: spec-driven

     Ready to create: proposal
     Use /ape:continue to create it, or /ape:ff to create all artifacts.
```

**Tips:**
- Use descriptive names: `add-feature`, `fix-bug`, `refactor-module`
- Avoid generic names like `update`, `changes`, `wip`
- Schema can also be set in project config (`apeworkflow/config.yaml`)
...
AI:  Change: add-dark-mode
     ...
     ✓ Created apeworkflow/changes/add-dark-mode/specs/ui/spec.md

     Now available: tasks
     Run /ape:continue to create the next artifact.
```
...
AI:  Fast-forwarding add-dark-mode...
     ...
     Ready for implementation. Run /ape:apply to begin.
```

**Tips:**
- Use when you have a clear picture of what you're building
- Faster than `/ape:continue` for straightforward changes
...
- Merges changes into main `apeworkflow/specs/` directory
...
AI:  Syncing add-dark-mode delta specs...
     ...
     Reading: apeworkflow/changes/add-dark-mode/specs/ui/spec.md
     Target:  apeworkflow/specs/ui/spec.md
     ...
     Merging...
     ✓ apeworkflow/specs/ui/spec.md updated

     Change remains active. Run /ape:archive when ready to complete.
```
...
- Moves change folder to `apeworkflow/changes/archive/YYYY-MM-DD-<name>/`
...
AI:  ✓ Synced specs to apeworkflow/specs/ui/spec.md
     ✓ Moved to apeworkflow/changes/archive/2025-01-24-add-dark-mode/
...
- Use `/ape:verify` first to catch issues
...
Guided onboarding through the complete ApeWorkflow workflow. An interactive tutorial using your actual codebase.
...
3. Creating a change (`/ape:new`)
4. Writing the proposal
5. Creating specs
6. Writing the design
7. Creating tasks
8. Implementing tasks (`/ape:apply`)
...
| Tool | Syntax Example |
|------|----------------|
| Claude Code | `/ape:propose`, `/ape:apply` |
| Cursor | `/ape-propose`, `/ape-apply` |
| Windsurf | `/ape-propose`, `/ape-apply` |
| Copilot (IDE) | `/ape-propose`, `/ape-apply` |
| Kimi CLI | Skill-based invocations such as `/skill:apeworkflow-propose`, `/skill:apeworkflow-apply-change` (no generated `ape-*` command files) |
| Trae | Skill-based invocations such as `/apeworkflow-propose`, `/apeworkflow-apply-change` (no generated `ape-*` command files) |
...
These commands use the older "all-at-once" workflow. They still work but APE commands are recommended.

| Command | What it does |
|---------|--------------|
| `/apeworkflow:proposal` | Create all artifacts at once (proposal, specs, design, tasks) |
| `/apeworkflow:apply` | Implement the change |
| `/apeworkflow:archive` | Archive the change |
...
**Migrating to APE:**
Legacy changes can be continued with APE commands. The artifact structure is compatible.
...
**Solutions:**
- Specify the change name explicitly: `/ape:apply add-dark-mode`
- Check that the change folder exists: `apeworkflow list`
...
**Solutions:**
- Run `apeworkflow status --change <name>` to see what's blocking
...
**Solutions:**
- List available schemas: `apeworkflow schemas`
- Check spelling of schema name
- Create the schema if it's custom: `apeworkflow schema init <name>`

### Commands not recognized

The AI tool doesn't recognize ApeWorkflow commands.

**Solutions:**
- Ensure ApeWorkflow is initialized: `apeworkflow init`
- Regenerate skills: `apeworkflow update`
...
**Solutions:**
- Add project context in `apeworkflow/config.yaml`
- Add per-artifact rules for specific guidance
- Provide more detail in your change description
- Use `/ape:continue` instead of `/ape:ff` for more control
