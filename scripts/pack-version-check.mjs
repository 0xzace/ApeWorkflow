...
function main() {
  ...
  try {
    log(`Packing @0xzace/apeworkflow@${expected}...`);
    const filename = npmPack();
    tgzPath = path.resolve(filename);
    log(`Created: ${tgzPath}`);

    work = mkdtempSync(path.join(tmpdir(), "apeworkflow-pack-check-"));
    log(`Temp dir: ${work}`);
    ...
    const actual = run(process.execPath, [binRel, "--version"], {
      cwd: work,
    }).trim();
