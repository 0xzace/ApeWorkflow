...
describe('PowerShellGenerator', () => {
	...
	describe('generate', () => {
		it('should generate valid PowerShell completion script with header', () => {
			...
			expect(script).toContain('# PowerShell completion script for ApeWorkflow CLI');
			expect(script).toContain('$apeworkflowCompleter = {');
			expect(script).toContain('Register-ArgumentCompleter');
		});

		it('should register argument completer for apeworkflow command', () => {
			const commands: CommandDefinition[] = [
				{
					name: 'init',
					description: 'Initialize ApeWorkflow',
			...
			expect(script).toContain('Register-ArgumentCompleter -CommandName apeworkflow');
			expect(script).toContain('-ScriptBlock $apeworkflowCompleter');
		});
		...
		it('should handle positional arguments for change-id', () => {
			...
			expect(script).toContain('Get-ApeWorkflowChanges');
		});
		...
		it('should handle positional arguments for spec-id', () => {
			...
			expect(script).toContain('Get-ApeWorkflowSpecs');
		});
		...
		it('should handle positional arguments for schema names', () => {
			...
			expect(script).toContain('Get-ApeWorkflowSchemas');
			expect(script).toContain('apeworkflow __complete schemas 2>$null');
		});
		...
		it('should generate dynamic completion helper for changes', () => {
			...
			expect(script).toContain('function Get-ApeWorkflowChanges');
			expect(script).toContain('apeworkflow __complete changes 2>$null');
		...
		it('should generate dynamic completion helper for specs', () => {
			...
			expect(script).toContain('function Get-ApeWorkflowSpecs');
			expect(script).toContain('apeworkflow __complete specs 2>$null');
		});
