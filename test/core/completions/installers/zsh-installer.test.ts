...
describe('ZshInstaller', () => {
  ...
  describe('getInstallationPath', () => {
    it('should return Oh My Zsh path when Oh My Zsh is installed', async () => {
      ...
      expect(result.path).toBe(path.join(testHomeDir, '.oh-my-zsh', 'custom', 'completions', '_apeworkflow'));
    });
    ...
    it('should return standard Zsh path when Oh My Zsh is not installed', async () => {
      ...
      expect(result.path).toBe(path.join(testHomeDir, '.zsh', 'completions', '_apeworkflow'));
    });
  });
  ...
  describe('install', () => {
    const testScript = '#compdef apeworkflow\n_apeworkflow() {\n  echo "test"\n}\n';
    ...
    it('should install to Oh My Zsh path when Oh My Zsh is present', async () => {
      ...
      expect(result.isOhMyZsh).toBe(true);
      expect(result.installedPath).toBe(path.join(ohMyZshPath, 'custom', 'completions', '_apeworkflow'));
    ...
    it('should install to standard Zsh path when Oh My Zsh is not present', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.isOhMyZsh).toBe(false);
      expect(result.installedPath).toBe(path.join(testHomeDir, '.zsh', 'completions', '_apeworkflow'));
    ...
    it('should backup existing file before overwriting', async () => {
      const targetPath = path.join(testHomeDir, '.zsh', 'completions', '_apeworkflow');
    ...
    it('should include fpath instructions for standard Zsh when auto-config is disabled', async () => {
      const originalEnv = process.env.APEWORKFLOW_NO_AUTO_CONFIG;
      process.env.APEWORKFLOW_NO_AUTO_CONFIG = '1';
      ...
      if (originalEnv === undefined) {
        ...
        process.env.APEWORKFLOW_NO_AUTO_CONFIG = originalEnv;
      }
    });
    ...
    it('should handle paths with spaces in .zshrc config', async () => {
      ...
      const testHomeDirWithSpaces = path.join(os.tmpdir(), `apeworkflow zsh test ${randomUUID()}`);
  ...
  describe('uninstall', () => {
    ...
    it('should remove from correct location for Oh My Zsh', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.message).toContain(path.join('.oh-my-zsh', 'custom', 'completions', '_apeworkflow'));
    });
  });
  ...
  describe('getInstallationInfo', () => {
    ...
    it('should return installation info when installed', async () => {
      ...
      expect(info.path).toContain('_apeworkflow');
  ...
  describe('configureZshrc', () => {
    ...
    it('should create .zshrc with markers and config when file does not exist', async () => {
      ...
      expect(content).toContain('# APEWORKFLOW:START');
      expect(content).toContain('# APEWORKFLOW:END');
      expect(content).toContain('# ApeWorkflow shell completions configuration');
    ...
    it('should prepend markers and config when .zshrc exists without markers', async () => {
      ...
      const configIndex = content.indexOf('# APEWORKFLOW:START');
    ...
    it('should preserve user content outside markers', async () => {
      ...
      expect(content).toContain(`fpath=("${completionsDir}" $fpath)`);
      expect(content).not.toContain('# Old ApeWorkflow config');
    });

    it('should return false when APEWORKFLOW_NO_AUTO_CONFIG is set', async () => {
      const originalEnv = process.env.APEWORKFLOW_NO_AUTO_CONFIG;
      process.env.APEWORKFLOW_NO_AUTO_CONFIG = '1';
  ...
  describe('removeZshrcConfig', () => {
    ...
    it('should remove markers and config when present', async () => {
      ...
      const content = [
        ...
        '',
        '# APEWORKFLOW:START',
        '# ApeWorkflow shell completions configuration',
      ...
      expect(newContent).not.toContain('# APEWORKFLOW:START');
      expect(newContent).not.toContain('# APEWORKFLOW:END');
      expect(newContent).not.toContain('ApeWorkflow shell completions');
    ...
    it('should handle invalid marker placement gracefully', async () => {
      ...
      await fs.writeFile(zshrcPath, '# APEWORKFLOW:END\n# APEWORKFLOW:START\n');
    ...
    it('should return true when only one marker is present', async () => {
      const zshrcPath = path.join(testHomeDir, '.zshrc');
      await fs.writeFile(zshrcPath, '# APEWORKFLOW:START\nsome config\n');
  ...
  describe('install with .zshrc auto-configuration', () => {
    ...
    it('should not configure .zshrc for Oh My Zsh', async () => {
      ...
      expect(content).not.toContain('# APEWORKFLOW:START');
  ...
  describe('uninstall with .zshrc cleanup', () => {
    ...
    it('should remove .zshrc config when uninstalling', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.message).toContain('Removed ApeWorkflow configuration from ~/.zshrc');
