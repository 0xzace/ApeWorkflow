...
describe('BashInstaller', () => {
  ...
  beforeEach(async () => {
    ...
    testHomeDir = path.join(os.tmpdir(), `apeworkflow-bash-test-${randomUUID()}`);
  ...
  describe('getInstallationPath', () => {
    it('should return standard bash-completion path', async () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.local', 'share', 'bash-completion', 'completions', 'apeworkflow'));
    });
  });
  ...
  describe('install', () => {
    const testScript = '# Bash completion script for ApeWorkflow CLI\n_apeworkflow_completion() {\n  echo "test"\n}\n';

    it('should install to bash-completion path', async () => {
      const result = await installer.install(testScript);

      expect(result.success).toBe(true);
      expect(result.installedPath).toBe(path.join(testHomeDir, '.local', 'share', 'bash-completion', 'completions', 'apeworkflow'));
    ...
    it('should backup existing file before overwriting', async () => {
      const targetPath = path.join(testHomeDir, '.local', 'share', 'bash-completion', 'completions', 'apeworkflow');
    ...
    it('should handle paths with spaces in .bashrc config', async () => {
      ...
      const testHomeDirWithSpaces = path.join(os.tmpdir(), `apeworkflow bash test ${randomUUID()}`);
  ...
  describe('removeBashrcConfig', () => {
    ...
    it('should remove markers and config when present', async () => {
      ...
      const content = [
        ...
        '  . ~/.local/share/bash-completion/completions/apeworkflow',
        'fi',
        '# APEWORKFLOW:END',
        '',
        'alias ll="ls -la"',
      ...
      expect(newContent).not.toContain('ApeWorkflow shell completions configuration');
