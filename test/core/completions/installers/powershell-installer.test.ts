...
describe('PowerShellInstaller', () => {
  ...
  beforeEach(async () => {
    testHomeDir = path.join(os.tmpdir(), `apeworkflow-powershell-test-${randomUUID()}`);
  ...
  describe('getInstallationPath', () => {
    it('should return path relative to profile directory', () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.config', 'powershell', 'ApeWorkflowCompletion.ps1'));
    });

    it('should work with custom PROFILE environment variable', () => {
      process.env.PROFILE = path.join(testHomeDir, 'custom', 'profile.ps1');
      const result = installer.getInstallationPath();
      expect(result).toBe(path.join(testHomeDir, 'custom', 'ApeWorkflowCompletion.ps1'));
    });
    ...
    it('should return Windows path when on Windows platform', () => {
      ...
      const result = installer.getInstallationPath();
      expect(result).toBe(path.join(testHomeDir, 'Documents', 'PowerShell', 'ApeWorkflowCompletion.ps1'));
    });
  });
  ...
  describe('configureProfile', () => {
    const mockScriptPath = '/path/to/ApeWorkflowCompletion.ps1';

    // Note: APEWORKFLOW_NO_AUTO_CONFIG check is now handled in the install() method,
    ...
    it.skipIf(process.platform === 'win32')('should skip configuration when script line already exists', async () => {
      ...
      const initialContent = [
        '# APEWORKFLOW:START - ApeWorkflow completion (managed block, do not edit manually)',
        `. "${mockScriptPath}"`,
        '# APEWORKFLOW:END',
        '',
        '# My custom config',
  ...
  describe('removeProfileConfig', () => {
    ...
    it('should remove content between markers', async () => {
      ...
      expect(content).not.toContain('# APEWORKFLOW:END');
      expect(content).not.toContain('# ApeWorkflow completions');
  ...
  describe('install', () => {
    const mockCompletionScript = `# PowerShell completion script for ApeWorkflow
$apeworkflowCompleter = {
    param($wordToComplete, $commandAst, $cursorPosition)
    # Completion logic here
}
Register-ArgumentCompleter -CommandName apeworkflow -ScriptBlock $apeworkflowCompleter
`;
    ...
    it('should install completion script for the first time', async () => {
      ...
      expect(result.installedPath).toContain('ApeWorkflowCompletion.ps1');
    ...
    // Note: APEWORKFLOW_NO_AUTO_CONFIG support was removed from PowerShell installer
    ...
    it('should handle installation with paths containing spaces', async () => {
      const spacedHomeDir = path.join(os.tmpdir(), `apeworkflow powershell test ${randomUUID()}`);
      ...
      expect(result.success).toBe(true);
      expect(result.installedPath).toContain('apeworkflow powershell test');
  ...
  describe('encoding preservation', () => {
    ...
    it('should preserve UTF-16 LE BOM when removing profile config', async () => {
      ...
      const textWithBlock = [
        '. "C:\\Code\\profile.ps1"',
        '# APEWORKFLOW:START',
        '. "/path/to/ApeWorkflowCompletion.ps1"',
        '# APEWORKFLOW:END',
        '',
      ].join('\n');
      ...
      // Verify content: original line kept, ApeWorkflow block removed
      const content = raw.subarray(2).toString('utf16le');
    ...
    it('should round-trip UTF-16 LE through install → uninstall without corruption', async () => {
      ...
      // Install adds the ApeWorkflow block
      ...
      // Uninstall removes the ApeWorkflow block
...
`;
    ...
    it('should handle both script and config removal', async () => {
      ...
      expect(profileContent).toContain('# APEWORKFLOW:START');
      ...
      expect(profileContentAfter).not.toContain('# APEWORKFLOW:START');
    });
