...
describe('migration', () => {
  ...
  beforeEach(async () => {
    projectDir = path.join(os.tmpdir(), `apeworkflow-migration-project-${randomUUID()}`);
    configHome = path.join(os.tmpdir(), `apeworkflow-migration-config-${randomUUID()}`);
  ...
  it('migrates to custom skills delivery when only managed skills are detected', async () => {
    await writeSkill(projectDir, 'apeworkflow-explore');
    await writeSkill(projectDir, 'apeworkflow-apply-change');
  ...
  it('ignores unknown custom skill and command files when scanning workflows', async () => {
    ...
    const customCommandPath = path.join(projectDir, '.claude', 'commands', 'ape', 'my-custom.md');
