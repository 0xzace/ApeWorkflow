...
describe('workspace foundation', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'apeworkflow-workspace-foundation-'));
...
`
  ...
  describe('path helpers', () => {
    it('exposes the workspace constants', () => {
      expect(WORKSPACE_METADATA_DIR_NAME).toBe('.apeworkflow-workspace');
    ...
    it('returns workspace file paths using platform-aware path helpers', () => {
      ...
      expect(getWorkspaceMetadataDir(workspaceRoot)).toBe(
        path.join(workspaceRoot, '.apeworkflow-workspace')
      );
      expect(getWorkspaceViewStatePath(workspaceRoot)).toBe(
        path.join(workspaceRoot, '.apeworkflow-workspace', 'view.yaml')
      );
    ...
    it('preserves Windows-style location strings when building workspace file paths', () => {
      ...
      expect(getWorkspaceViewStatePath(workspaceRoot)).toBe(
        'D:\\repos\\platform-workspace\\.apeworkflow-workspace\\view.yaml'
      );
    });
    ...
    it('uses getGlobalDataDir for managed workspace and registry locations', () => {
      ...
      expect(getManagedWorkspacesDir()).toBe(path.join(tempDir, 'apeworkflow', 'workspaces'));
      expect(getManagedWorkspaceRoot('platform')).toBe(
        path.join(tempDir, 'apeworkflow', 'workspaces', 'platform')
      );
      expect(getWorkspaceRegistryPath()).toBe(
        path.join(tempDir, 'apeworkflow', 'workspaces', 'registry.yaml')
      );
    });
    ...
    it('uses the Linux data-dir fallback under the managed workspaces directory', () => {
      ...
      expect(getManagedWorkspacesDir({ globalDataDir: dataDir })).toBe(
        '/home/tabish/.local/share/apeworkflow/workspaces'
      );
    });
    ...
    it('uses the native Windows data-dir fallback under the managed workspaces directory', () => {
      ...
      expect(getManagedWorkspacesDir({ globalDataDir: dataDir })).toBe(
        'C:\\Users\\Tabish\\AppData\\Local\\apeworkflow\\workspaces'
      );
    });
  ...
  describe('workspace folder detection', () => {
    ...
    it('does not mistake repo-local apeworkflow projects for coordination workspaces', async () => {
      const repoRoot = path.join(tempDir, 'repo');
      fs.mkdirSync(path.join(repoRoot, 'apeworkflow', 'changes', 'add-feature'), {
        recursive: true,
      });
      fs.mkdirSync(path.join(repoRoot, 'apeworkflow', 'specs'), { recursive: true });

      await expect(findWorkspaceRoot(path.join(repoRoot, 'apeworkflow', 'changes'))).resolves.toBe(
        null
      );
    });
    ...
    it('ignores foreign root workspace.yaml files in repo-local projects', async () => {
      ...
      const nestedDir = path.join(repoRoot, 'apeworkflow', 'changes', 'add-feature');
      fs.mkdirSync(nestedDir, { recursive: true });
      fs.writeFileSync(
...
`
    ...
    it('ignores unmarked root view state even when it is ApeWorkflow-shaped', async () => {
...
`
    ...
    it('writes canonical view state inside the ApeWorkflow metadata directory', async () => {
    ...
    it('detects a workspace even when a linked path has no repo-local apeworkflow state', async () => {
      const workspaceRoot = createWorkspaceRoot();
...
`;
      ...
      expect(refreshed.match(/APEWORKFLOW:WORKSPACE-GUIDANCE:START/gu)).toHaveLength(1);
      expect(applyWorkspaceGuidanceBlock('# Team Notes\n')).toContain(
        'APEWORKFLOW:WORKSPACE-GUIDANCE:START -->'
      );
    });
...
`);
    ...
    it('reads the local registry from the standard registry path', async () => {
      const globalDataDir = path.join(tempDir, 'data', 'apeworkflow');
