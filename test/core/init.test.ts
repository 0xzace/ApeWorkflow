...
describe('InitCommand', () => {
  ...
  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `apeworkflow-init-test-${Date.now()}`);
    await fs.mkdir(testDir, { recursive: true });
    originalEnv = { ...process.env };
    // Use a temp dir for global config to avoid reading real config
    configTempDir = path.join(os.tmpdir(), `apeworkflow-config-init-${Date.now()}`);
  ...
  describe('execute with --tools flag', () => {
    it('should create ApeWorkflow directory structure', async () => {
      const initCommand = new InitCommand({ tools: 'claude', force: true });

      await initCommand.execute(testDir);

      const apeworkflowPath = path.join(testDir, 'apeworkflow');
      expect(await directoryExists(apeworkflowPath)).toBe(true);
      expect(await directoryExists(path.join(apeworkflowPath, 'specs'))).toBe(true);
      expect(await directoryExists(path.join(apeworkflowPath, 'changes'))).toBe(true);
      expect(await directoryExists(path.join(apeworkflowPath, 'changes', 'archive'))).toBe(true);
    });
    ...
    it('should create config.yaml with default schema', async () => {
      ...
      const configPath = path.join(testDir, 'apeworkflow', 'config.yaml');
    ...
    it('should create skills in Cursor skills directory', async () => {
      ...
      const skillFile = path.join(testDir, '.cursor', 'skills', 'apeworkflow-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    });
    ...
    it('should create skills in Windsurf skills directory', async () => {
      ...
      const skillFile = path.join(testDir, '.windsurf', 'skills', 'apeworkflow-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    });
    ...
    it('should support Kimi CLI as an adapterless skills-only tool', async () => {
      ...
      const skillFile = path.join(testDir, '.kimi', 'skills', 'apeworkflow-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    ...
    it('should create skills for multiple tools at once', async () => {
      ...
      const claudeSkill = path.join(testDir, '.claude', 'skills', 'apeworkflow-explore', 'SKILL.md');
      const cursorSkill = path.join(testDir, '.cursor', 'skills', 'apeworkflow-explore', 'SKILL.md');
    ...
    it('should select all tools with --tools all option', async () => {
      ...
      const claudeSkill = path.join(testDir, '.claude', 'skills', 'apeworkflow-explore', 'SKILL.md');
      const cursorSkill = path.join(testDir, '.cursor', 'skills', 'apeworkflow-explore', 'SKILL.md');
      const windsurfSkill = path.join(testDir, '.windsurf', 'skills', 'apeworkflow-explore', 'SKILL.md');
    ...
    it('should skip tool configuration with --tools none option', async () => {
      ...
      // Should create ApeWorkflow structure but no skills
      const apeworkflowPath = path.join(testDir, 'apeworkflow');
      expect(await directoryExists(apeworkflowPath)).toBe(true);
    ...
    it('should not create config.yaml if it already exists', async () => {
      ...
      const configPath = path.join(apeworkflowDir, 'config.yaml');
    ...
    it('should handle non-existent target directory', async () => {
      ...
      const apeworkflowPath = path.join(newDir, 'apeworkflow');
      expect(await directoryExists(apeworkflowPath)).toBe(true);
    });
  ...
  describe('skill content validation', () => {
    ...
    it('should include propose skill instructions', async () => {
      ...
      const skillFile = path.join(testDir, '.claude', 'skills', 'apeworkflow-propose', 'SKILL.md');
      const content = await fs.readFile(skillFile, 'utf-8');

      expect(content).toContain('name: apeworkflow-propose');
    });
    ...
    it('should include apply-change skill instructions', async () => {
      ...
      const skillFile = path.join(testDir, '.claude', 'skills', 'apeworkflow-apply-change', 'SKILL.md');
      const content = await fs.readFile(skillFile, 'utf-8');

      expect(content).toContain('name: apeworkflow-apply-change');
    });
  ...
  describe('command generation', () => {
    it('should generate Claude Code commands with correct format', async () => {
      ...
      const cmdFile = path.join(testDir, '.claude', 'commands', 'ape', 'explore.md');
    ...
    it('should generate Cursor commands with correct format', async () => {
      ...
      const cmdFile = path.join(testDir, '.cursor', 'commands', 'ape-explore.md');
      expect(await fileExists(cmdFile)).toBe(true);
      ...
      vi.spyOn(fs, 'writeFile').mockImplementation(
        async (filePath: any, ...args: any[]) => {
          if (
            ...
            filePath.includes('.apeworkflow-test-')
          ) {
  ...
  describe('tool-specific adapters', () => {
    it('should generate Gemini CLI commands as TOML files', async () => {
      ...
      const cmdFile = path.join(testDir, '.gemini', 'commands', 'ape', 'explore.toml');
      expect(await fileExists(cmdFile)).toBe(true);
    ...
    it('should generate Windsurf commands', async () => {
      ...
      const cmdFile = path.join(testDir, '.windsurf', 'workflows', 'ape-explore.md');
      expect(await fileExists(cmdFile)).toBe(true);
    });
    ...
    it('should generate Continue prompt files', async () => {
      ...
      const cmdFile = path.join(testDir, '.continue', 'prompts', 'ape-explore.prompt');
      expect(await fileExists(cmdFile)).toBe(true);

      const content = await fs.readFile(cmdFile, 'utf-8');
      expect(content).toContain('name: ape-explore');
    ...
    it('should generate Cline workflow files', async () => {
      ...
      const cmdFile = path.join(testDir, '.clinerules', 'workflows', 'ape-explore.md');
      expect(await fileExists(cmdFile)).toBe(true);
    });
    ...
    it('should generate GitHub Copilot prompt files', async () => {
      ...
      const cmdFile = path.join(testDir, '.github', 'prompts', 'ape-explore.prompt.md');
      expect(await fileExists(cmdFile)).toBe(true);
    });
  });
});
...
describe('InitCommand - profile and detection features', () => {
  ...
  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `apeworkflow-init-profile-test-${Date.now()}`);
    await fs.mkdir(testDir, { recursive: true });
    originalEnv = { ...process.env };
    // Use a temp dir for global config to avoid polluting real config
    configTempDir = path.join(os.tmpdir(), `apeworkflow-config-test-${Date.now()}`);
    await fs.mkdir(configTempDir, { recursive: true });
    process.env.XDG_CONFIG_HOME = configTempDir;
    vi.spyOn(console, 'log').mockImplementation(() => {});
    confirmMock.mockReset();
  ...
  it('should use --profile flag to override global config', async () => {
    ...
    const newChangeSkill = path.join(testDir, '.claude', 'skills', 'apeworkflow-new-change', 'SKILL.md');
  ...
  it('should auto-cleanup legacy artifacts in non-interactive mode without --force', async () => {
    ...
    await fs.writeFile(path.join(legacyDir, 'ape-propose.md'), 'legacy content');
    ...
    expect(await fileExists(path.join(legacyDir, 'ape-propose.md'))).toBe(false);
  ...
  it('should preselect configured tools but not directory-detected tools in extend mode', async () => {
    // Simulate existing ApeWorkflow project (extend mode).
    await fs.mkdir(path.join(testDir, 'apeworkflow'), { recursive: true });

    // Configured with ApeWorkflow
    ...
    // Directory detected only (not configured with ApeWorkflow)
  ...
  it('should preselect detected tools for first-time interactive setup', async () => {
    // First-time init: no apeworkflow/ directory and no configured ApeWorkflow skills.
    await fs.mkdir(path.join(testDir, '.github'), { recursive: true });
    await fs.writeFile(path.join(testDir, '.github', 'copilot-instructions.md'), '');
