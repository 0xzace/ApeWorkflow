...
describe('UpdateCommand', () => {
  ...
  beforeEach(async () => {
    ...
    // Create apeworkflow directory
    const apeworkflowDir = path.join(testDir, 'apeworkflow');
    await fs.mkdir(apeworkflowDir, { recursive: true });
  ...
  describe('basic validation', () => {
    it('should throw error if apeworkflow directory does not exist', async () => {
      // Remove apeworkflow directory
      await fs.rm(path.join(testDir, 'apeworkflow'), {
        recursive: true,
        force: true,
      });

      await expect(updateCommand.execute(testDir)).rejects.toThrow(
        "No ApeWorkflow directory found. Run 'apeworkflow init' first."
      );
    });
...
`;
      ...
      expect(updatedSkill).toContain('name: apeworkflow-explore');
    ...
    it('should update core profile skill files when tool is configured', async () => {
      ...
      await fs.writeFile(
        path.join(skillsDir, 'apeworkflow-explore', 'SKILL.md'),
        'old content'
      );
  ...
  describe('command updates', () => {
    it('should update ape commands for configured Claude tool', async () => {
      ...
      // Check ape command files were created
  ...
  describe('multi-tool support', () => {
    it('should update multiple configured tools', async () => {
      ...
      await fs.mkdir(path.join(claudeSkillsDir, 'apeworkflow-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'apeworkflow-explore', 'SKILL.md'),
        'old'
      );

      // Set up Cursor
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'apeworkflow-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(cursorSkillsDir, 'apeworkflow-explore', 'SKILL.md'),
        'old'
      );
      ...
      expect(claudeSkill).toContain('name: apeworkflow-explore');
      ...
      expect(cursorSkill).toContain('name: apeworkflow-explore');
    ...
    it('should update Qwen tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(qwenSkillsDir, 'apeworkflow-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(qwenSkillsDir, 'apeworkflow-explore', 'SKILL.md'),
        'old'
      );

      await updateCommand.execute(testDir);

      // Check Qwen command format (TOML) - Qwen uses flat path structure: ape-<id>.toml
    ...
    it('should update Windsurf tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(windsurfSkillsDir, 'apeworkflow-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(windsurfSkillsDir, 'apeworkflow-explore', 'SKILL.md'),
        'old'
      );
  ...
  describe('skill content validation', () => {
    ...
    it('should include proper instructions in skill files', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'apeworkflow-apply-change'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'apeworkflow-apply-change', 'SKILL.md'),
        'old'
      );
...
`
    ...
    it('should update all tools when force is used with mixed versions', async () => {
      ...
      const { version } = await import('../../package.json');
      const claudeSkillDir = path.join(testDir, '.claude', 'skills', 'apeworkflow-explore');
...
`
      ...
      const cursorSkillDir = path.join(testDir, '.cursor', 'skills', 'apeworkflow-explore');
...
`
  ...
  describe('version tracking', () => {
    ...
    it('should only update tools that need updating', async () => {
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'apeworkflow-explore', 'SKILL.md');
  ...
  describe('legacy cleanup', () => {
    it('should detect and auto-cleanup legacy files with --force flag', async () => {
      ...
      // Create legacy CLAUDE.md with ApeWorkflow markers
      const legacyContent = `${APEWORKFLOW_MARKERS.start}
...
`;
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Upgrading to the new ApeWorkflow')
      );

      // Should show marker removal message (config files are never deleted, only have markers removed)
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed ApeWorkflow markers from CLAUDE.md')
      );
...
`;
    ...
    it('should cleanup legacy slash command directories with --force', async () => {
      ...
      const legacyCommandDir = path.join(testDir, '.claude', 'commands', 'apeworkflow');
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed .claude/commands/apeworkflow/')
      );
    ...
    it('should cleanup legacy apeworkflow/AGENTS.md with --force', async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'apeworkflow-explore'), {
        recursive: true,
      ...
      // Create legacy apeworkflow/AGENTS.md
      await fs.writeFile(
        path.join(testDir, 'apeworkflow', 'AGENTS.md'),
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed apeworkflow/AGENTS.md')
      );
    ...
    it('should not show legacy cleanup messages when no legacy files exist', async () => {
      ...
      const hasLegacyMessage = calls.some(call =>
        call.includes('Upgrading to the new ApeWorkflow')
      );
    ...
    it('should remove ApeWorkflow marker block from mixed content files', async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'apeworkflow-explore'), {
        recursive: true,
      ...
      // Create CLAUDE.md with mixed content (user content + ApeWorkflow markers)
...
`;
      ...
      expect(updatedContent).not.toContain(APEWORKFLOW_MARKERS.start);
      expect(updatedContent).not.toContain(APEWORKFLOW_MARKERS.end);
  ...
  describe('legacy tool upgrade', () => {
    it('should upgrade legacy tools to new skills with --force', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('/ape:new')
      );
    ...
    it('should upgrade multiple legacy tools with --force', async () => {
      ...
      await fs.writeFile(
        path.join(testDir, '.claude', 'commands', 'apeworkflow', 'proposal.md'),
        'content'
      );

      await fs.mkdir(path.join(testDir, '.cursor', 'commands'), { recursive: true });
      await fs.writeFile(
        path.join(testDir, '.cursor', 'commands', 'apeworkflow-proposal.md'),
        'content'
      );
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'apeworkflow-explore', 'SKILL.md');
      const cursorSkillFile = path.join(testDir, '.cursor', 'skills', 'apeworkflow-explore', 'SKILL.md');
    ...
    it('should not upgrade legacy tools already configured', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'apeworkflow-explore'), { recursive: true });
      await fs.writeFile(
        path.join(skillsDir, 'apeworkflow-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should upgrade only unconfigured legacy tools when mixed', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'apeworkflow-explore'), { recursive: true });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'apeworkflow-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should create only effective profile skills when upgrading legacy tools', async () => {
      ...
      const nonCoreSkill = path.join(skillsDir, 'apeworkflow-new-change', 'SKILL.md');
    ...
    it('should create commands when upgrading legacy tools', async () => {
      ...
      // New ape commands should be created
      const commandsDir = path.join(testDir, '.claude', 'commands', 'ape');
      const exploreCmd = path.join(commandsDir, 'explore.md');
      const exists = await FileSystemUtils.fileExists(exploreCmd);
      expect(exists).toBe(true);
    ...
    it('should keep the visible command surface when upgrading legacy tools', async () => {
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'apeworkflow-propose', 'SKILL.md')
      )).toBe(false);
  ...
  describe('profile-aware updates', () => {
    it('should generate only profile workflows when custom profile is set', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'apeworkflow-explore'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'apeworkflow-explore', 'SKILL.md'), 'old');
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'apeworkflow-new-change', 'SKILL.md')
      )).toBe(true);
    ...
    it('should suggest core preset when custom profile preserves the old core workflow set', async () => {
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(testDir, '.claude', 'skills', 'apeworkflow-sync-specs', 'SKILL.md')
      )).toBe(false);
      expect(await FileSystemUtils.fileExists(
        path.join(testDir, '.claude', 'commands', 'ape', 'sync.md')
      )).toBe(false);
...
`
    ...
    it('should remove workflows outside profile during update sync', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'apeworkflow-new-change'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'apeworkflow-new-change', 'SKILL.md'), 'old');
      const extraCommandFile = path.join(testDir, '.claude', 'commands', 'ape', 'new.md');
  ...
  describe('new tool detection', () => {
    it('should detect new tool directories not currently configured', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'apeworkflow-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'apeworkflow-explore', 'SKILL.md'), 'old');
      ...
      const hasNewToolMessage = calls.some(call =>
        call.includes("Detected new tool: Cursor. Run 'apeworkflow init' to add it.")
      );
    ...
    it('should consolidate multiple new tools into one message', async () => {
      ...
      expect(consolidatedCalls[0]).toContain("Run 'apeworkflow init' to add them.");
  ...
  describe('scanInstalledWorkflows', () => {
    it('should detect installed workflows across tools', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'apeworkflow-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'apeworkflow-explore', 'SKILL.md'), 'content');
      await fs.mkdir(path.join(claudeSkillsDir, 'apeworkflow-apply-change'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'apeworkflow-apply-change', 'SKILL.md'), 'content');
    ...
    it('should return union of workflows across multiple tools', async () => {
      ...
      // Cursor has apply
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'apeworkflow-apply-change'), { recursive: true });
      await fs.writeFile(path.join(cursorSkillsDir, 'apeworkflow-apply-change', 'SKILL.md'), 'content');
