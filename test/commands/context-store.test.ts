...
async function runContextStoreCommand(args: string[]): Promise<void> {
  ...
  await program.parseAsync(['node', 'apeworkflow', 'context-store', ...args]);
}
...
describe('context-store command', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'apeworkflow-context-store-command-'));
  ...
  it('sets up a context store in the managed local data directory without Git in non-interactive JSON mode', async () => {
    ...
    expect(payload.created_files).toEqual(['.apeworkflow-store/store.yaml']);
  ...
  it('rejects registry id and alias path conflicts', async () => {
    ...
    fs.rmSync(path.join(firstRoot, '.apeworkflow-store'), { recursive: true, force: true });
