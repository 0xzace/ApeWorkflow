...
describe('workspace open initiative views', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'apeworkflow-workspace-initiative-'));
  ...
  it('creates a default local view for an initiative and returns a JSON receipt', async () => {
    ...
    expect(fs.existsSync(path.join(workspaceRoot, '.apeworkflow-workspace'))).toBe(true);
