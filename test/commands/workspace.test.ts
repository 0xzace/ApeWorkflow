...
describe('workspace command', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'apeworkflow-workspace-command-'));
  ...
  function createFakeExecutable(name: string): { binDir: string; logPath: string } {
    ...
    fs.writeFileSync(
      recorderPath,
      "const fs = require('node:fs');\nfs.writeFileSync(process.env.APEWORKFLOW_FAKE_OPEN_LOG, JSON.stringify({ cwd: process.cwd(), args: process.argv.slice(2) }));\n"
    );

    const posixExecutable = path.join(binDir, name);
    fs.writeFileSync(posixExecutable, '#!/bin/sh\nnode "$APEWORKFLOW_FAKE_OPEN_RECORDER" "$@"\n');
  ...
  function envWithFakeExecutable(fake: { binDir: string; logPath: string }): NodeJS.ProcessEnv {
    return {
      ...
      APEWORKFLOW_FAKE_OPEN_RECORDER: path.join(fake.binDir, 'record-launch.cjs'),
      APEWORKFLOW_FAKE_OPEN_LOG: fake.logPath,
    };
  }
  ...
  function writeGlobalConfig(config: Record<string, unknown>): void {
    const configDir = path.join(configHome, 'apeworkflow');
  ...
  it('sets up a workspace with required links, records local state, and lists it through ls', async () => {
    ...
    mkdir('repos/api/apeworkflow/specs');
    ...
    expect(setup.workspace.links).toEqual([
      expect.objectContaining({
        name: 'api',
        path: expectedApi,
        repo_specs_path: path.join(expectedApi, 'apeworkflow', 'specs'),
        status: [],
      }),
      expect.objectContaining({
        name: 'checkout',
    ...
    expect(fs.existsSync(getWorkspaceRegistryPath({ globalDataDir: path.join(dataHome, 'apeworkflow') }))).toBe(false);
    ...
    expect(fs.readFileSync(path.join(workspaceRoot, 'AGENTS.md'), 'utf-8')).toContain(
      'ApeWorkflow Workspace Guidance'
    );
      ...
      expect.objectContaining({
        ...
        skipped: [
          expect.objectContaining({
            ...
            message: expect.stringContaining('apeworkflow workspace update --tools <ids>'),
          }),
        ],
      })
    );
  ...
  it('installs profile-selected workspace skills in the workspace root only', async () => {
    ...
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'apeworkflow-apply-change', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'apeworkflow-archive-change', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'apeworkflow-propose', 'SKILL.md'))).toBe(false);
  ...
  it('updates stored workspace skills from the current workspace and clears profile drift', async () => {
    ...
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'apeworkflow-apply-change', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'apeworkflow-verify-change', 'SKILL.md'))).toBe(true);
    ...
    expect(parseJson(drift).workspace.status).toContainEqual(
      expect.objectContaining({
        ...
        fix: 'apeworkflow workspace update --workspace profile-sync',
      })
    );
    ...
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'apeworkflow-propose', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'apeworkflow-explore', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'apeworkflow-sync-specs', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'apeworkflow-archive-change', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'apeworkflow-verify-change'))).toBe(false);
  ...
  it('does not route apeworkflow update through workspace update from a workspace root', async () => {
    const api = mkdir('repos/api');
    const linkedEntriesBefore = fs.readdirSync(api).sort();
    writeGlobalConfig({
      profile: 'custom',
    ...
    expect(`${update.stdout}\n${update.stderr}`).toContain('Run `apeworkflow workspace update`');
    ...
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'apeworkflow-propose', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'apeworkflow-sync-specs', 'SKILL.md'))).toBe(false);
    expect(fs.readdirSync(api).sort()).toEqual(linkedEntriesBefore);
    expect(fs.existsSync(path.join(api, '.codex'))).toBe(false);
  });
  ...
  it('updates repo-local project targets nested under a workspace without touching workspace state', async () => {
    ...
    fs.mkdirSync(path.join(nestedRepo, 'apeworkflow'), { recursive: true });
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'apeworkflow-apply-change', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'apeworkflow-propose', 'SKILL.md'))).toBe(false);
    ...
    expect(`${update.stdout}\n${update.stderr}`).not.toContain('Run `apeworkflow workspace update`');
    expect(update.stdout).not.toContain('Workspace update complete');
    expect(fs.readFileSync(getWorkspaceViewStatePath(workspaceRoot), 'utf-8')).toBe(workspaceStateBefore);
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'apeworkflow-propose', 'SKILL.md'))).toBe(false);
  });
  ...
  it('does not touch workspace state when updating repo-local projects with foreign workspace.yaml', async () => {
    ...
    expect(fs.existsSync(path.join(existingWorkspaceRoot, '.codex', 'skills', 'apeworkflow-apply-change', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(existingWorkspaceRoot, '.codex', 'skills', 'apeworkflow-propose', 'SKILL.md'))).toBe(false);
    ...
    fs.mkdirSync(path.join(repoRoot, 'apeworkflow'), { recursive: true });
...
`;
  ...
  it('does not update a workspace passed to apeworkflow update even when another workspace is known', async () => {
    ...
    expect(`${update.stdout}\n${update.stderr}`).toContain('Run `apeworkflow workspace update`');
    expect(update.stdout).not.toContain('Workspace update complete');
    expect(update.stdout).not.toContain('Multiple ApeWorkflow workspaces are known');
    ...
    expect(fs.existsSync(path.join(first.workspace.root, '.codex', 'skills', 'apeworkflow-propose', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(second.workspace.root, '.codex', 'skills', 'apeworkflow-propose', 'SKILL.md'))).toBe(false);
  });
  ...
  it('supports named and flag-selected workspace updates with explicit agent changes', async () => {
    ...
    expect(fs.existsSync(path.join(workspaceRoot, '.claude', 'skills', 'apeworkflow-apply-change', 'SKILL.md'))).toBe(true);
    ...
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'apeworkflow-apply-change'))).toBe(false);
  ...
  it('does not remove unmanaged skill directories that collide with ApeWorkflow workflow names', async () => {
    const api = mkdir('repos/api');
    writeGlobalConfig({
      profile: 'custom',
      delivery: 'skills',
    ...
    const workspaceRoot = setup.workspace.root;
    const collidingSkillDir = path.join(workspaceRoot, '.codex', 'skills', 'apeworkflow-verify-change');
  ...
  it('does not record workspace skills as applied when an update fails', async () => {
    ...
    const workspaceRoot = setup.workspace.root;
    const blockingSkillPath = path.join(workspaceRoot, '.codex', 'skills', 'apeworkflow-propose');
...
`
  ...
  it('removes a partially created workspace when setup fails after creating the root', async () => {
    ...
    const globalDataDir = path.join(dataHome, 'apeworkflow');
  ...
  it('fails setup cleanly for missing automation inputs and JSON without no-interactive', async () => {
    ...
    expect(noWorkspaces.stdout).toContain("No ApeWorkflow workspaces found. Run 'apeworkflow workspace setup' first.");
  ...
  it('links monorepo folders without editing the linked folder', async () => {
    ...
    expect(fs.existsSync(path.join(packageDir, 'apeworkflow'))).toBe(false);
  ...
  it('drops deleted managed workspace roots from scanned workspace selection', async () => {
    const api = mkdir('repos/api');
    const setup = await setupWorkspace('platform', [`api=${api}`]);
    const registryPath = getWorkspaceRegistryPath({ globalDataDir: path.join(dataHome, 'apeworkflow') });
...
`;
  ...
  it('fails non-interactive ambiguous workspace selection in human output mode', async () => {
    ...
    expect(doctor.exitCode).toBe(1);
    expect(doctor.stderr).toContain('Multiple ApeWorkflow workspaces are known.');
    expect(doctor.stderr).toContain('Pass --workspace <name>.');
    expect(doctor.stderr).toContain('apeworkflow workspace doctor --workspace <name>');
  });
  ...
  it('opens a workspace through VS Code editor and agent overrides without changing stored preference', async () => {
    ...
    expect(codexLaunch.args).toEqual([
      ...
      '--add-dir',
      expectedApi,
      'Open this ApeWorkflow workspace.',
    ]);
  ...
  it('reports workspace open selection errors', async () => {
    ...
    expect(noKnown.exitCode).toBe(1);
    expect(noKnown.stderr).toContain("No known ApeWorkflow workspaces. Run 'apeworkflow workspace setup' first.");
...
`
  ...
  it('prints readable human output for setup, list, and doctor', async () => {
    ...
    expect(setup.stdout).toContain('ApeWorkflow workspaces (1)');
    ...
    expect(list.exitCode).toBe(0);
    expect(list.stdout).toContain('ApeWorkflow workspaces (1)');
