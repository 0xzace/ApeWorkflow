...
async function runWorkspaceCommand(args: string[]): Promise<void> {
  ...
  await program.parseAsync(['node', 'apeworkflow', 'workspace', ...args]);
}
...
describe('workspace command interactive flows', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'apeworkflow-workspace-interactive-'));
  ...
  it('asks which agents get ApeWorkflow skills and preselects the preferred opener', async () => {
    const api = mkdir('repos/api');
    const binDir = mkdir('bin');
    ...
    searchableMultiSelectMock.mockImplementationOnce(async (options: {
      ...
      expect(options.message).toBe('Which agents should get ApeWorkflow skills in this workspace?');
