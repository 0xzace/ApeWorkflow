...
describe('artifact-workflow CLI commands', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'apeworkflow-artifact-workflow-'));
  ...
  async function createTestChange(
    ...
    if (artifacts.includes('plans')) {
      // 中文注释：apply 流程改为优先读取 plans/ 下的计划文件，这里用固定日期文件名模拟真实计划。
      const plansDir = path.join(changeDir, 'plans');
      await fs.mkdir(plansDir, { recursive: true });
      await fs.writeFile(
        path.join(plansDir, `2026-06-17-${changeName}.md`),
        '## Plan\n- [ ] Task 1'
      );
    }
  ...
  describe('status command', () => {
    ...
    it('shows complete status when all artifacts are done', async () => {
      await createTestChange('complete-change', ['proposal', 'design', 'specs', 'tasks', 'plans']);
    ...
    it('exits gracefully when no changes exist', async () => {
      ...
      expect(result.stdout).toContain('apeworkflow new change');
    });
  ...
  describe('new change command', () => {
    ...
    it('creates workspace-planning changes under the workspace root without touching linked repos', async () => {
      ...
      await fs.mkdir(path.join(api, 'apeworkflow', 'specs'), { recursive: true });
      ...
      const metadata = await fs.readFile(path.join(changeDir, '.apeworkflow.yaml'), 'utf-8');
      ...
      await expect(fs.stat(path.join(api, 'apeworkflow', 'changes'))).rejects.toMatchObject({
        code: 'ENOENT',
      });
    });
  ...
  describe('instructions apply command', () => {
    it('shows apply instructions for spec-driven schema with plan files', async () => {
      await createTestChange('apply-change', ['proposal', 'design', 'specs', 'tasks', 'plans']);
    ...
    it('shows blocked state when plan files are missing', async () => {
      // Create the core planning artifacts and tasks, but omit plans/ so apply waits for the real execution source.
      await createTestChange('blocked-apply', ['proposal', 'design', 'specs', 'tasks']);

      const result = await runCLI(['instructions', 'apply', '--change', 'blocked-apply'], {
        cwd: tempDir,
      });
      expect(result.exitCode).toBe(0);
      expect(result.stdout).toContain('Missing artifacts: plans');
    });

    it('outputs JSON for apply instructions', async () => {
      await createTestChange('json-apply', ['proposal', 'design', 'specs', 'tasks', 'plans']);
      ...
      expect(json.contextFiles.tasks).toBeUndefined();
      expect(json.contextFiles.plans).toEqual([
        canonical(path.join(changesDir, 'json-apply', 'plans', '2026-06-17-json-apply.md')),
      ]);
...
`
      ...
      await fs.mkdir(path.dirname(specPath), { recursive: true });
      await fs.writeFile(path.join(changeDir, '.apeworkflow.yaml'), 'schema: glob-test\n');
    ...
    it('shows all_done state when all tasks are complete', async () => {
      const changeDir = await createTestChange('done-apply', ['proposal', 'design', 'specs', 'tasks', 'plans']);
      // 中文注释：覆盖 plans 里的计划文件，模拟所有计划项都已完成。
      await fs.writeFile(path.join(changeDir, 'plans', '2026-06-17-done-apply.md'), '## Plan\n- [x] Task 1\n- [x] Task 2');
    ...
    it('spec-driven schema uses apply block configuration', async () => {
      ...
      await createTestChange('apply-config-test', ['proposal', 'design', 'specs', 'tasks', 'plans']);
      ...
      const json = JSON.parse(result.stdout);
      // spec-driven schema has apply block with plans, so should be ready
      expect(json.schemaName).toBe('spec-driven');
      expect(json.state).toBe('ready');
    });
    ...
    it('fallback: requires all artifacts when schema has no apply block', async () => {
      ...
      const noApplySchemaDir = path.join(userDataDir, 'apeworkflow', 'schemas', 'no-apply');
...
`;
    ...
    it('fallback: ready when all artifacts exist for schema without apply block', async () => {
      ...
      const noApplySchemaDir = path.join(userDataDir, 'apeworkflow', 'schemas', 'no-apply-full');
      const templatesDir = path.join(noApplySchemaDir, 'templates');
      await fs.mkdir(templatesDir, { recursive: true });
...
`;
  ...
  describe('experimental command (deprecated alias for init)', () => {
    ...
    it('creates skills for Claude tool', async () => {
      ...
      const skillFile = path.join(tempDir, '.claude', 'skills', 'apeworkflow-explore', 'SKILL.md');
    ...
    it('creates skills for Cursor tool', async () => {
      ...
      // Verify skill files were created
      const skillFile = path.join(tempDir, '.cursor', 'skills', 'apeworkflow-explore', 'SKILL.md');
      const stat = await fs.stat(skillFile);
      expect(stat.isFile()).toBe(true);

      // Verify commands were created with Cursor format
      const commandFile = path.join(tempDir, '.cursor', 'commands', 'ape-explore.md');
    ...
    it('creates skills for Windsurf tool', async () => {
      ...
      // Verify skill files were created
      const skillFile = path.join(tempDir, '.windsurf', 'skills', 'apeworkflow-explore', 'SKILL.md');
      const stat = await fs.stat(skillFile);
      expect(stat.isFile()).toBe(true);
    });
  });
  ...
  describe('project config integration', () => {
    describe('new change uses config schema', () => {
      it('creates change with schema from project config', async () => {
        ...
        // Note: changesDir is already at tempDir/apeworkflow/changes (created in beforeEach)
        await fs.writeFile(
          path.join(tempDir, 'apeworkflow', 'config.yaml'),
          'schema: spec-driven\n'
        );
        ...
        const metadataPath = path.join(changesDir, 'test-change', '.apeworkflow.yaml');
      ...
      it('CLI schema overrides config schema', async () => {
        // Create project config with spec-driven schema
        // Note: apeworkflow directory already exists (from changesDir creation in beforeEach)
        await fs.writeFile(
          path.join(tempDir, 'apeworkflow', 'config.yaml'),
          'schema: spec-driven\n'
        );
        ...
        const metadataPath = path.join(changesDir, 'override-test', '.apeworkflow.yaml');
        const metadata = await fs.readFile(metadataPath, 'utf-8');
        expect(metadata).toContain('schema: spec-driven');
      }, 60000);
    });
...
`
    ...
    describe('backwards compatibility', () => {
      ...
      it('changes with metadata work without config file', async () => {
        ...
        await fs.writeFile(
          path.join(changeDir, '.apeworkflow.yaml'),
          'schema: spec-driven\ncreated: "2025-01-05"\n'
        );
