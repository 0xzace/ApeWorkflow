...
async function runConfigCommand(args: string[]): Promise<void> {
  ...
  await program.parseAsync(['node', 'apeworkflow', 'config', ...args]);
}
...
describe('config profile interactive flow', () => {
  ...
  function setupDriftedProjectArtifacts(projectDir: string): void {
    fs.mkdirSync(path.join(projectDir, 'apeworkflow'), { recursive: true });
    const exploreSkillPath = path.join(projectDir, '.claude', 'skills', 'apeworkflow-explore', 'SKILL.md');
    fs.mkdirSync(path.dirname(exploreSkillPath), { recursive: true });
    fs.writeFileSync(exploreSkillPath, 'name: apeworkflow-explore\n', 'utf-8');
  }
  ...
  function addExtraVerifyWorkflowArtifacts(projectDir: string): void {
    const verifySkillPath = path.join(projectDir, '.claude', 'skills', 'apeworkflow-verify-change', 'SKILL.md');
    fs.mkdirSync(path.dirname(verifySkillPath), { recursive: true });
    fs.writeFileSync(verifySkillPath, 'name: apeworkflow-verify-change\n', 'utf-8');

    const verifyCommandPath = path.join(projectDir, '.claude', 'commands', 'ape', 'verify.md');
  ...
  function setupWorkspaceState(
    ...
    const metadataDir = path.join(workspaceRoot, '.apeworkflow-workspace');
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(os.tmpdir(), `apeworkflow-config-profile-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it('selecting current values only should be a no-op and should not ask apply', async () => {
    ...
    fs.mkdirSync(path.join(tempDir, 'apeworkflow'), { recursive: true });
    select.mockResolvedValueOnce('delivery');
  ...
  it('changed config should ask to apply to the current workspace and print workspace guidance when declined', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('Config updated. Run `apeworkflow workspace update` to apply it to workspace-local skills.');
  });
  ...
  it('confirmed workspace apply should run workspace update instead of repo-local update', async () => {
    ...
    expect(execSync).toHaveBeenCalledWith('npx apeworkflow workspace update', {
      stdio: 'inherit',
      cwd: process.cwd(),
    });
    expect(execSync).not.toHaveBeenCalledWith('npx apeworkflow update', expect.anything());
  });
  ...
  it('no-op inside a workspace should warn when workspace skills drift', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('apeworkflow workspace update'));
  });
