...
describe('telemetry/index', () => {
  ...
  describe('isTelemetryEnabled', () => {
    it('should return false when APEWORKFLOW_TELEMETRY=0', () => {
      process.env.APEWORKFLOW_TELEMETRY = '0';
    ...
    it('should return true when no opt-out is set', () => {
      delete process.env.APEWORKFLOW_TELEMETRY;
      delete process.env.DO_NOT_TRACK;
      delete process.env.CI;
      expect(isTelemetryEnabled()).toBe(true);
    });

    it('should prioritize APEWORKFLOW_TELEMETRY=0 over other settings', () => {
      process.env.APEWORKFLOW_TELEMETRY = '0';
      delete process.env.DO_NOT_TRACK;
      delete process.env.CI;
      expect(isTelemetryEnabled()).toBe(false);
  ...
  describe('trackCommand', () => {
    ...
    it('should construct PostHog with bounded silent-failure settings', async () => {
      ...
      expect(PostHog).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          host: 'https://edge.apeworkflow.dev',
          flushAt: 1,
          flushInterval: 0,
          fetchRetryCount: 0,
          requestTimeout: 1000,
    ...
    it('should return a synthetic success response when fetch throws a network error', async () => {
      ...
      const response = await fetchFn('https://edge.apeworkflow.dev/batch/', { method: 'POST' });
